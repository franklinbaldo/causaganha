...
class R2DuckDBClient:
    ...
    def query_snapshot(self, snapshot_key: str, query: str) -> pd.DataFrame:
        ...
        if snapshot_key.endswith(".zst"):
            logger.warning(
                "Compressed snapshots require local decompression for queries"
            )

            # Download and decompress temporarily
            temp_path = Path("temp_query_snapshot.duckdb")
            try:
                decompressed_path = self.r2_storage.download_snapshot(
                    snapshot_key, temp_path
                )
    ...
    def compare_snapshots(
        self, snapshot1_key: str, snapshot2_key: str, metric: str = "mu"
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            # Cleanup
            self.conn.execute("DETACH snap1") if "snap1" in str(
                self.conn.execute("SHOW DATABASES").df()
            ) else None
            self.conn.execute("DETACH snap2") if "snap2" in str(
                self.conn.execute("SHOW DATABASES").df()
            ) else None
    ...
    def get_temporal_analysis(
        self, days_back: int = 30, advogado_id: Optional[str] = None
    ) -> pd.DataFrame:
        """
        ...
        relevant_snapshots = [
            s
            for s in snapshots
            if datetime.fromisoformat(s["last_modified"].replace("Z", "+00:00"))
            >= cutoff_date
        ]

        if len(relevant_snapshots) < 2:
            logger.warning(
                "Only %d snapshots found in last %d days",
                len(relevant_snapshots),
                days_back,
            )
            return pd.DataFrame()

        logger.info(
            "Analyzing %d snapshots over %d days", len(relevant_snapshots), days_back
        )
        ...
        comparison = self.compare_snapshots(
            oldest_snapshot["key"], newest_snapshot["key"]
        )
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["rankings", "stats", "compare", "trends"],
