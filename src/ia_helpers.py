...
try:
    ...
    print("Warning: Could not load from config.py, using fallback IA/DB config values in ia_helpers.")
...
async def execute_ia_command_async(ia_command_args: List[str], log_output: bool = True) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            if log_output and log_main_ops:
                typer.echo(f"✅ IA OK: {command_str_for_logging.split(' ')[1] if len(command_str_for_logging.split(' ')) > 1 else ''}")
            ...
            if log_output and log_main_ops: # Always log errors if main logging is on
                typer.echo(f"❌ IA FAIL (code {result.returncode}): {command_str_for_logging}", err=True)
                if result.stderr: typer.echo(f"   Stderr: {result.stderr.strip()}", err=True)
                if result.stdout: typer.echo(f"   Stdout: {result.stdout.strip()}", err=True)
            return False
    except subprocess.TimeoutExpired:
        if log_main_ops: typer.echo(f"❌ IA TIMEOUT: {command_str_for_logging}", err=True)
        return False
    except Exception as e:
        if log_main_ops: typer.echo(f"❌ IA EXCEPTION {command_str_for_logging}: {type(e).__name__} - {e}", err=True)
        return False
...
async def execute_ia_upload_async(
    ...
    if not local_filepath.exists():
        if log_output and log_main_ops: typer.echo(f"❌ UploadFAIL: File {local_filepath} not found.", err=True)
        return False
...
async def archive_diario_to_master_item(
    ...
    if log_main_ops:
        typer.echo(f"📦 Archiving '{local_pdf_path.name}' to IA master '{master_id}' as '{remote_ia_full_path}'")

    upload_successful = await execute_ia_upload_async(
        target_ia_id=master_id, local_filepath=local_pdf_path, remote_filename=remote_ia_full_path
    )

    if upload_successful:
        if log_main_ops: typer.echo(f"✅ Archived OK: '{master_id}/{remote_ia_full_path}'")
        return master_id, remote_ia_full_path
    else:
        if log_main_ops: typer.echo(f"❌ ArchiveFAIL: '{local_pdf_path.name}' to master item.", err=True)
        return None, None


async def download_ia_file_async(
    item_id: str, remote_filename_on_ia: str, destination_dir: Path, log_output: bool = True
) -> Optional[Path]:
    ...
    if log_output and log_main_ops:
        typer.echo(f"⬇️  Downloading '{remote_filename_on_ia}' from IA '{item_id}' to '{local_filepath}'")
    ...
    if success and local_filepath.exists() and local_filepath.is_file():
        if log_output and log_main_ops: typer.echo(f"✅ Download OK: '{local_filepath}'")
        return local_filepath
    else:
        if log_output and log_main_ops:
            typer.echo(f"❌ DownloadFAIL: '{remote_filename_on_ia}' from IA '{item_id}'. Target '{local_filepath}' not found/not file.", err=True)
        if local_filepath.exists():
            try: local_filepath.unlink()
            except OSError: pass
        return None
...
async def update_ia_file_level_metadata_summary(
    ...
    if log_main_ops:
        typer.echo(f"🔄 MetaUPDATE: For '{file_remote_path}' in '{master_ia_id}/{summary_json_filename}'")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir_name:
        ...
        if downloaded_summary_filepath and downloaded_summary_filepath.exists():
            try:
                ...
                if log_main_ops: typer.echo(f"📄 MetaDL OK: Parsed '{summary_json_filename}'.")
            except Exception as e: # Catch JSONDecodeError and other read errors
                if log_main_ops: typer.echo(f"⚠️ MetaDL WARN: Corrupt/unreadable '{summary_json_filename}': {e}. Creating new.", err=True)
        else:
            if log_main_ops: typer.echo(f"📄 MetaDL INFO: '{summary_json_filename}' not found on IA. Creating new.")

    current_summary_data[file_remote_path] = new_file_metadata_entry
    if log_main_ops: typer.echo(f"📊 MetaLocalUPDATE: Entry for '{file_remote_path}'.")

    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".json", encoding='utf-8') as tmp_upload_file:
        json.dump(current_summary_data, tmp_upload_file, ensure_ascii=False, indent=2)
        temp_upload_filepath = Path(tmp_upload_file.name)

    if log_main_ops: typer.echo(f"📝 MetaLocalSAVE: Updated summary at '{temp_upload_filepath}'")

    upload_successful = await execute_ia_upload_async(
        master_ia_id, temp_upload_filepath, summary_json_filename, log_output=False
    )
    try: temp_upload_filepath.unlink()
    except OSError as e:
        if log_main_ops: typer.echo(f"⚠️ MetaCleanupWARN: Could not del temp upload file '{temp_upload_filepath}': {e}", err=True)

    if upload_successful:
        if log_main_ops: typer.echo(f"✅ MetaUpload OK: '{summary_json_filename}' to '{master_ia_id}'.")
        return True
    else:
        if log_main_ops: typer.echo(f"❌ MetaUploadFAIL: '{summary_json_filename}' to '{master_ia_id}'.", err=True)
        return False
