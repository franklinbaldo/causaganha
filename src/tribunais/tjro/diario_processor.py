    ...
    for diario in diarios:
        try:
            ...
            entry = {
                ...
                'number': int(number),
                'sufix': sufix,
                'ia_identifier': f"tjro-diario-{year}-{month.zfill(2)}-{day.zfill(2)}" + (f"-{sufix.lower()}" if sufix else ""),
                'metadata': {
                    'title': f"Diário da Justiça TJRO - {day}/{month}/{year}" + (f" ({sufix})" if sufix else ""),
