...
def _populate_decisoes_from_analysis_result(
    ...
    if isinstance(analysis_result, list):  # If result is a list of decisions
        ...
        isinstance(analysis_result, dict) and "decisions" in analysis_result
    ):  # If result is a dict containing a 'decisions' list
        decisions_to_store = analysis_result.get("decisions", [])
    elif (
        isinstance(analysis_result, dict) and "numero_processo" in analysis_result
    ):  # If result is a single decision dict
        decisions_to_store = [analysis_result]

    source_json_file_name = (
        f"job_{job_id}_{original_filename_for_json_source}.json"  # Conceptual name
    )
            ...
            try:
                ...
                """,
                    [
                        ...
                        json.dumps(
                            decision_data
                        ),  # Store full decision as raw_json_data
                    ],
                )
                _LOG_.debug(
...
async def analyze_stage_logic(
    ...
    async def _proc(
        jid, iurl, mid, rf, an_s, tc, ofn, idt_s
    ):  # job_id, item_url, master_id, remote_file, analysis_str, trib_code, orig_fname, item_date_str
...
def _show_rating_stats():
    try:
        ...
        if tl:
            typer.echo(f"🏆 Top 10 Lawyers (min {mg} games):")
...
@app.command("db")
def database_cmd(
    action: str = typer.Argument(..., help="migrate,status,backup,reset"),
...
def চেষ্টা_করা(func, *args, **kwargs):  # "try" in Bengali
    try:
        ...
        _LOG_.debug(f"চেষ্টা_করা caught:{e}")


# Alias for cleanup, used above in exception handlers
try_cleanup = চেষ্টা_করা
