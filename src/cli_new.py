                ...
                try:
                    async def _dl():
                        async with aiohttp.ClientSession()as sess,sess.get(durl)as rsp:
                            ...
                            sfn="".join(c if c.isalnum()or c in('_','-')else'_'for c in Path(ofname).stem)
                            sf=f"{sfn}.pdf"
                            dlp=tmp_d/sf
                            dlp.write_bytes(cnt)
...
async def analyze_stage_logic(limit:Optional[int]=None,force_analysis:bool=False,max_concurrent_analyses:int=2):
    ...
    if force_analysis:
        s_cond.extend(["status='analyzed'","status='ia_metadata_updated'","status='failed_analysis'","status='failed_metadata_sync'"])
    ...
    if not items:
        ...
        return 0,0,0,0 # Added one more 0 for decisions_stored
    _LOG_.info(f"Analyze:Found {len(items)}.")
    typer.echo(f"🔍 Processing {len(items)} items...")
    xtr=GeminiExtractor()
    llm_ok,meta_ok,nok,dec_stored_count=0,0,0,0 # decisions_stored_count
    ...
    async def _proc(jid,iurl,mid,rf,an_s,tc,ofn,idt_s): # job_id, item_url, master_id, remote_file, analysis_str, trib_code, orig_fname, item_date_str
        ...
        if need_llm:
            if not xtr.gemini_configured:
                ...
                if not ana_data:
                    nok+=1
                    db.conn.execute("UPDATE job_queue SET status='failed_analysis',error_message='Gemini N/A'WHERE id=?",[jid])
                    return
            else:
                ...
                with tempfile.TemporaryDirectory(prefix=f"cg_an_{jid}_")as tmp_d_s:
                    ...
                    try:
                        ...
                        llm_ok+=1
                        db.conn.execute("UPDATE job_queue SET analyze_result=?,status='analyzed',updated_at=CURRENT_TIMESTAMP WHERE id=?",[json.dumps(ana_data),jid])
                        ...
                        nok+=1
                        db.conn.execute("UPDATE job_queue SET status='failed_analysis',error_message=? WHERE id=?",[f"LLM fail:{e}",jid])
                        return
                    finally:
        ...
        if ana_data:
            ...
            if sync_ok:
                meta_ok+=1
                db.conn.execute("UPDATE job_queue SET ia_metadata_synced=TRUE,status='ia_metadata_updated',updated_at=CURRENT_TIMESTAMP WHERE id=?",[jid])
                _LOG_.info(f"IA meta sync OK job {jid}.")
                typer.echo(f"   ✅ IA meta sync OK:{rf}.")
            else:
                nok+=1
                db.conn.execute("UPDATE job_queue SET ia_metadata_synced=FALSE,status='failed_metadata_sync',error_message='IA summary fail'WHERE id=?",[jid])
            ...
            nok+=1
            db.conn.execute("UPDATE job_queue SET status='failed_analysis',error_message='No analysis data for sync'WHERE id=?",[jid])
    if items:
        tasks = [
...
def diario_cmd(action:str=typer.Argument(...,help="list,stats"),tribunal:Optional[str]=typer.Option(None),status:Optional[str]=typer.Option(None),limit:int=typer.Option(20)):
    if action=="stats":
        ...
        for r in rs:
            typer.echo(f" ID:{r[0]} Date:{r[2]} Trib:{r[3].upper()} Status:{r[4]} IA:{r[5]}/{r[6]} URL:{r[1]}")
    else:
...
def _db_reset(force:bool):
    ...
    if not dbf.exists():
        ...
        _db_migrate()
        return
    if not force and not typer.confirm(f"⚠️ DELETE {dbf}? IRREVERSIBLE. Confirm?",abort=False):
