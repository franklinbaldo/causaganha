...
def fetch_tjro_pdf(date_str: str, dry_run: bool = False, verbose: bool = False):
    ...
    try:
        ...
        try: date_obj = datetime.date.fromisoformat(date_str)
        except ValueError: logger.error("Could not parse date '%s'", date_str); return None
    pdf_path = _real_fetch_tjro_pdf(date_obj)
    if pdf_path:
        try:
            from gdrive import upload_file_to_gdrive
            upload_file_to_gdrive(pdf_path)
        except Exception as e: logger.error(f"Failed to upload {pdf_path} to GDrive: {e}")
    return pdf_path
...
class GeminiExtractor:
    ...
    def extract_and_save_json(self, pdf_path: Path, output_json_dir: Path = None, dry_run: bool = False):
        ...
        output_json_dir = Path(output_json_dir) if output_json_dir else pdf_path.parent / "json_extracted"
        ...
        if dry_run:
            ...
            with open(output_json_path, "w", encoding="utf-8") as f: json.dump({"file_name_source": pdf_path.name, "simulated": True}, f, indent=2)
...
def extract_command(args):
    ...
    output_dir = Path(args.output_json_dir) if args.output_json_dir else Path(args.pdf_file).parent
    extractor = GeminiExtractor(verbose=args.verbose)
    json_path = extractor.extract_and_save_json(pdf_path=Path(args.pdf_file), output_json_dir=output_dir, dry_run=args.dry_run)
...
def _update_ratings_logic(logger: logging.Logger, dry_run: bool):
    ...
    try:
        ...
        if "mu" in ratings_df.columns: ratings_df["mu"] = ratings_df["mu"].astype(float)
        if "sigma" in ratings_df.columns: ratings_df["sigma"] = ratings_df["sigma"].astype(float)
        ...
        ratings_df = pd.DataFrame(columns=["mu", "sigma", "total_partidas"]).set_index(pd.Index([], name="advogado_id"))
    ...
    for col in ["mu", "sigma", "total_partidas"]:
        if col not in ratings_df.columns:
            if col == "mu": ratings_df[col] = DEFAULT_MU
            elif col == "sigma": ratings_df[col] = DEFAULT_SIGMA
            else: ratings_df[col] = 0
    ...
    if not json_files_to_process: logger.info(f"No JSON files in {json_input_dir} to process.")
    ...
    for json_path in json_files_to_process:
        ...
        except Exception as e: logger.error(f"Error reading/decoding {json_path.name}: {e}. Skipping."); continue

        decisions_in_file = loaded_content.get("decisions", []) if isinstance(loaded_content, dict) else \
                            loaded_content if isinstance(loaded_content, list) else \
                            [loaded_content] if loaded_content else []
        ...
        for decision_data in decisions_in_file:
            ...
            team_a_ids = sorted(list(set(normalize_lawyer_name(name) for name in raw_advs_polo_ativo if normalize_lawyer_name(name))))
            team_b_ids = sorted(list(set(normalize_lawyer_name(name) for name in raw_advs_polo_passivo if normalize_lawyer_name(name))))

            if not team_a_ids or not team_b_ids: logger.warning(f"Missing lawyers in {json_path.name} ({decision_data.get('numero_processo')}). Skipping."); continue
            if set(team_a_ids) == set(team_b_ids): logger.info(f"Identical teams in {decision_data.get('numero_processo')}. Skipping."); continue

            resultado_str_raw = decision_data.get("resultado", "").lower()
            match_outcome = MatchResult.DRAW
            if resultado_str_raw in ["procedente", "provido", "confirmada"]: match_outcome = MatchResult.WIN_A
            elif resultado_str_raw in ["improcedente", "negado_provimento", "reformada"]: match_outcome = MatchResult.WIN_B

            team_a_ratings_before = [CREATE_RATING_FROM_MU_SIGMA_FUNC(ratings_df.loc[adv_id, "mu"], ratings_df.loc[adv_id, "sigma"]) if adv_id in ratings_df.index else CREATE_NEW_RATING_FUNC() for adv_id in team_a_ids]
            team_b_ratings_before = [CREATE_RATING_FROM_MU_SIGMA_FUNC(ratings_df.loc[adv_id, "mu"], ratings_df.loc[adv_id, "sigma"]) if adv_id in ratings_df.index else CREATE_NEW_RATING_FUNC() for adv_id in team_b_ids]

            partida_team_a_ratings_before_dict = {adv_id: (r.mu, r.sigma) for adv_id, r in zip(team_a_ids, team_a_ratings_before)}
            partida_team_b_ratings_before_dict = {adv_id: (r.mu, r.sigma) for adv_id, r in zip(team_b_ids, team_b_ratings_before)}

            new_team_a_ratings, new_team_b_ratings = UPDATE_RATINGS_FUNC(RATING_MODEL_INSTANCE, team_a_ratings_before, team_b_ratings_before, match_outcome)
            ...
            for i, adv_id in enumerate(team_a_ids):
                ...
                ratings_df.loc[adv_id, ["mu", "sigma", "total_partidas"]] = [new_team_a_ratings[i].mu, new_team_a_ratings[i].sigma, current_partidas + 1]
            for i, adv_id in enumerate(team_b_ids):
                current_partidas = ratings_df.loc[adv_id, "total_partidas"] if adv_id in ratings_df.index else 0
                ratings_df.loc[adv_id, ["mu", "sigma", "total_partidas"]] = [new_team_b_ratings[i].mu, new_team_b_ratings[i].sigma, current_partidas + 1]

            logger.debug(f"OpenSkill updated: A:{[f'{r.mu:.1f}±{r.sigma:.1f}' for r in new_team_a_ratings]} vs B:{[f'{r.mu:.1f}±{r.sigma:.1f}' for r in new_team_b_ratings]}")
            partidas_history.append({
                "data_partida": decision_data.get("data_decisao", datetime.date.today().isoformat()),
                "equipe_a_ids": ",".join(team_a_ids), "equipe_b_ids": ",".join(team_b_ids),
                ...
                "ratings_equipe_a_depois": json.dumps({adv_id: (r.mu, r.sigma) for adv_id, r in zip(team_a_ids, new_team_a_ratings)}),
                "ratings_equipe_b_depois": json.dumps({adv_id: (r.mu, r.sigma) for adv_id, r in zip(team_b_ids, new_team_b_ratings)}),
                "numero_processo": decision_data.get("numero_processo"),
            })
            file_had_valid_decisions = True
        if file_had_valid_decisions: processed_files_paths.append(json_path)

    if not dry_run:
        if not ratings_df.empty:
            try:
                if ratings_df.index.name is None: ratings_df.index.name = "advogado_id"
                ratings_df.sort_values(by=["mu", "sigma"], ascending=[False, True]).to_csv(ratings_file)
                logger.info(f"Ratings saved to {ratings_file}")
            except Exception as e: logger.error(f"Error saving ratings to {ratings_file}: {e}")
        else: logger.info("Ratings DataFrame empty. Not saving.")
        if partidas_history:
            try:
                pd.DataFrame(partidas_history).to_csv(partidas_file, index=False)
                logger.info(f"Partidas history saved to {partidas_file}")
            except Exception as e: logger.error(f"Error saving partidas to {partidas_file}: {e}")
        else: logger.info("No new partidas to save.")
        if processed_files_paths:
            processed_json_dir.mkdir(parents=True, exist_ok=True)
            for p_file in processed_files_paths:
                try: shutil.move(str(p_file), str(processed_json_dir / p_file.name)); logger.info(f"Moved {p_file.name} to {processed_json_dir}")
...
def run_command(args):
    ...
    pdf_path = fetch_tjro_pdf(date_str=args.date, dry_run=args.dry_run, verbose=args.verbose)
    if not pdf_path: logger.error(f"'collect' failed for {args.date}. Aborting."); return
    ...
    extractor = GeminiExtractor(verbose=args.verbose)
    json_output_path = extractor.extract_and_save_json(pdf_path=pdf_path, output_json_dir=extract_output_dir, dry_run=args.dry_run)
    if not json_output_path: logger.error(f"'extract' failed for {pdf_path}. 'run' partially completed."); return
...
def archive_command(args):
    ...
    if args.dry_run:
        logger.info(f"DRY-RUN: Archive: type={args.archive_type}, date={args.date or 'today'}, db={args.db_path}"); return
    try:
        from archive_db import DatabaseArchiver, IAConfig
        from datetime import datetime, date as DateObject
        snap_date = datetime.strptime(args.date, "%Y-%m-%d").date() if args.date else DateObject.today()
    ...
    except Exception as e: logger.error(f"Archive command failed: {e}", exc_info=args.verbose)
...
def main():
    ...
    cmd_definitions = [
        ("collect", "Downloads documents.", collect_command, [("--date", {"required": True, "help": "Date (YYYY-MM-DD)."}), ("--dry-run", {"action": "store_true"})]),
        ("extract", "Extracts PDF to JSON.", extract_command, [("--pdf_file", {"required": True, "type": Path, "help": "PDF path."}), ("--output_json_dir", {"type": Path}), ("--dry-run", {"action": "store_true"})]),
        ("update", "Updates ratings from JSONs.", update_command, [("--dry-run", {"action": "store_true"})]),
        ("archive", "Archives database.", archive_command, [("--date", {"help": "Snapshot date (YYYY-MM-DD, default: today)."}), ("--archive-type", {"choices": ["weekly", "monthly", "quarterly"], "default": "weekly"}), ("--db-path", {"type": Path, "default": Path("data/causaganha.duckdb")}), ("--dry-run", {"action": "store_true"})]),
        ("run", "Full pipeline: collect, extract, update.", run_command, [("--date", {"required": True, "help": "Date (YYYY-MM-DD)."}), ("--output_json_dir", {"type": Path, "help": "JSON out dir for extract."}), ("--dry-run", {"action": "store_true"})])
    ]
