...
class PJeAPIClient:
    ...
    async def get_intimations_by_court(
        ...
        while True:
            ...
            # If we've fetched everything available, stop
            if len(all_intimations) >= total_count:
                logger.info("all_pages_fetched",
                           total=len(all_intimations))
                break

            # If the current page didn't return a full page, we are likely done,
            # but comparing with total_count is safer.
            # If items < limit_per_page, it's the last page.
