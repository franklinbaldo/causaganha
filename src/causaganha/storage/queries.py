...
DEPRECATED: Use IntimationRepository in src/causaganha/storage/repository.py instead.
...
async def store_analysis_result(con: BaseBackend, result: dict[str, Any]) -> None:
    ...
    Deprecated. Use AnalysisRepository.store_analysis_results_batch.
    """
    repo = AnalysisRepository(con)
    await repo.store_analysis_results_batch([result])
