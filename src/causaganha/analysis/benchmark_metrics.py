"""Invariant-winner benchmark metrics ÔÇö gate + conditional decomposition.

The surface ``outcome`` label is *posture-dependent*: a first-instance
``procedente`` and an appellate ``recurso do r├®u n├úo provido`` describe the
...
won ÔÇö and injects label noise that caps achievable accuracy regardless of
model quality.

This module scores the **invariant**: the winning *polo* (``A`` = autor /
plaintiff, ``P`` = passivo / defendant, ``draw`` = autocomposi├º├úo), which is
...
- **Gate** ÔÇö "is this a ratable merits decision?" Interlocut├│rias, despachos
...
- **Conditional** ÔÇö "given a ratable decision, which polo won?" Scored only
...
def to_winner_polo(outcome: str, recorrente_polo: str | None = None) -> str:
    ...
    Args:
        outcome: Surface outcome label (e.g. ``procedente``, ``n├úo provido``).
        recorrente_polo: ``A``/``P``/``None`` ÔÇö who filed the appeal. Required
