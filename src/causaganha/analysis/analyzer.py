...
class DecisionAnalyzer:
    ...
    def __init__(self, model_name: str = "gemini-2.5-flash", provider: str = "google-gla"):
        ...
        self.agent = Agent(
            f"{provider}:{model_name}", result_type=DecisionAnalysis, system_prompt=system_prompt,
        )

        logger.info("analyzer_initialized", model=model_name, provider=provider)

    async def analyze_pdf(
        self, pdf_url: str, intimation_id: int | None = None,
    ) -> DecisionAnalysis:
        ...
        try:
            ...
            result = await self.agent.run(
                f"Analyze this judicial decision PDF: {pdf_url}", message_history=[],
            )
            ...
            logger.error("analysis_failed", intimation_id=intimation_id, url=pdf_url, error=str(e))
            raise

    async def analyze_batch(
        self, pdf_urls: list[str], intimation_ids: list[int] | None = None,
    ) -> list[Any]:
        """Analyze multiple PDFs concurrently.

        Returns:
            List of results (DecisionAnalysis or Exception) corresponding to input order.
        """
        ...
        # Create tasks
        tasks = [self.analyze_pdf(url, int_id) for url, int_id in zip(pdf_urls, intimation_ids)]
