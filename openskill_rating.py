...
def create_rating(os_model: PlackettLuce, mu: float = None, sigma: float = None, name: str = None) -> OpenSkillRating:
    """
...
def rate_teams(os_model: PlackettLuce,
    ...
    if result == 'win_a':
        ...
        raise ValueError(f"Unknown match result: {result}. Expected 'win_a', 'win_b', 'draw', 'partial_a', or 'partial_b'.")
...
if __name__ == '__main__':
    ...
    print(f"Example model initialized with: mu={example_model.mu:.2f}, sigma={example_model.sigma:.2f}, beta={example_model.beta:.2f}, tau={example_model.tau:.4f}")

    # Initial ratings for first scenario (Team A wins)
    player1_ex1 = create_rating(example_model, name="Player1_ex1")
    player2_ex1 = create_rating(example_model, mu=30.0, sigma=example_model.sigma, name="Player2_ex1")
    player3_ex1 = create_rating(example_model, name="Player3_ex1")

    print(f"Initial {player1_ex1.name}: mu={player1_ex1.mu:.2f}, sigma={player1_ex1.sigma:.2f}")
    print(f"Initial {player2_ex1.name}: mu={player2_ex1.mu:.2f}, sigma={player2_ex1.sigma:.2f}")
    print(f"Initial {player3_ex1.name}: mu={player3_ex1.mu:.2f}, sigma={player3_ex1.sigma:.2f}")
    ...
    print("\nTeam A wins:")
    updated_team_a_ex1, updated_team_b_ex1 = rate_teams(example_model, team_a_ex1, team_b_ex1, 'win_a')
    p1_updated_ex1, p2_updated_ex1 = updated_team_a_ex1
    p3_updated_ex1 = updated_team_b_ex1[0]
    print(f"Updated {p1_updated_ex1.name}: mu={p1_updated_ex1.mu:.2f}, sigma={p1_updated_ex1.sigma:.2f}")
    print(f"Updated {p2_updated_ex1.name}: mu={p2_updated_ex1.mu:.2f}, sigma={p2_updated_ex1.sigma:.2f}")
    print(f"Updated {p3_updated_ex1.name}: mu={p3_updated_ex1.mu:.2f}, sigma={p3_updated_ex1.sigma:.2f}")

    # Fresh ratings for Draw scenario
    player1_ex2 = create_rating(example_model, name="Player1_ex2")
    player2_ex2 = create_rating(example_model, mu=30.0, sigma=example_model.sigma, name="Player2_ex2")
    player3_ex2 = create_rating(example_model, name="Player3_ex2")
    team_a_ex2 = [player1_ex2, player2_ex2]
    team_b_ex2 = [player3_ex2]

    print("\nDraw:")
    updated_team_a_draw, updated_team_b_draw = rate_teams(example_model, team_a_ex2, team_b_ex2, 'draw')
    ...
    player2_ex3 = create_rating(example_model, mu=30.0, sigma=example_model.sigma, name="Player2_ex3")
    ...
    updated_team_a_partial, updated_team_b_partial = rate_teams(example_model, team_a_ex3, team_b_ex3, 'partial_a', partial_play_tau=0.7)
    p1_partial, p2_partial = updated_team_a_partial
    p3_partial = updated_team_b_partial[0]

    print(f"Updated {p1_partial.name}: mu={p1_partial.mu:.2f}, sigma={p1_partial.sigma:.2f}")
    print(f"Updated {p2_partial.name}: mu={p2_partial.mu:.2f}, sigma={p2_partial.sigma:.2f}")
    print(f"Updated {p3_partial.name}: mu={p3_partial.mu:.2f}, sigma={p3_partial.sigma:.2f}")
    ...
    print("\nPlayer 1 wins:")
    updated_p1_team, updated_p3_team = rate_teams(example_model, team_p1_sc2, team_p3_sc2, 'win_a')
    p1_1v1_win = updated_p1_team[0]
    p3_1v1_lose = updated_p3_team[0]
    print(f"Updated {p1_1v1_win.name}: mu={p1_1v1_win.mu:.2f}, sigma={p1_1v1_win.sigma:.2f}")
    print(f"Updated {p3_1v1_lose.name}: mu={p3_1v1_lose.mu:.2f}, sigma={p3_1v1_lose.sigma:.2f}")
    ...
    if win_probabilities:
        print(f"Win probability for {player1_sc2.name}: {win_probabilities[0]*100:.2f}%")
        print(f"Win probability for {player3_sc2.name}: {win_probabilities[1]*100:.2f}%")
