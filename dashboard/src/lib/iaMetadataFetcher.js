...
import { fetchWithRetry } from './fetchData.js';
...
export function getExpectedDays(year) {
  const now = new Date();
 ...
 */
function tribunalFromItemId(identifier, year) {
  const prefix = 'djen-';
  const suffix = `-${year}`;
 ...
 *
 * @param {number} year - Year to check (e.g. 2026)
 * @param {function} onProgress - Callback(done, total, results) — called once when done
 * @param {object} options - { useCache: true, tribunals: TRIBUNAIS }
 * @returns {Promise<object[]>} Array of { tribunal, fileCount, expectedDays, percentage, notFound }
 */
export async function fetchAllTribunalMetadata(year, onProgress, options = {}) {
  ...
  const results = tribunals.map(tribunal => {
