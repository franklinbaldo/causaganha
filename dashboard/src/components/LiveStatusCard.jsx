...
export function LiveStatusCard({ stats }) {
  ...
  return (
    <div className={clsx("cyber-card relative overflow-hidden border-t-4", borderColor)} role="status" aria-live="polite">
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-sm text-cyber-muted uppercase tracking-widest mb-1.5 font-medium">System Status</h2>
          <div className="flex items-center gap-3">
            {isSuccess ? <CheckCircle className="w-7 h-7 text-cyber-primary" aria-hidden="true" /> : <XCircle className="w-7 h-7 text-cyber-danger" aria-hidden="true" />}
            <span className={clsx("text-3xl font-bold tracking-tight", statusColor)}>
              {isSuccess ? 'OPERATIONAL' : 'SYSTEM FAULT'}
            </span>
          </div>
          <div className="mt-3 text-sm text-cyber-muted flex flex-wrap items-center gap-x-6 gap-y-1.5 font-medium">
        ...
        <div className="text-right hidden sm:block">
             <div className="text-sm text-cyber-muted mb-1 font-medium">NEXT RUN</div>
             <div className="text-2xl font-mono text-cyber-warning animate-pulse" aria-label="Next run in approximately 5 minutes">~05:00</div>
        </div>
      </div>
