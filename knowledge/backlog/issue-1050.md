...
- **Lote 17** (rodada 0hjgmk, PR #1574, aberta concorrentemente a esta
  rodada mas mesclada primeiro): 5 documentos, TJBA/574460085,
  TJMA/42736393, TJMA/42730832, TJCE/363647616, TJCE/363657243 (um
  sexto candidato, TJBA/574460088, foi descartado antes da anotação por
  ser quase-duplicata de TJBA/574460085, `difflib.SequenceMatcher.ratio()=0.98`).
  `document_count` 138->143, lote train-only (teto val/test inalterado
  em 21/21). Encontrou e corrigiu um bug real no script de
  remapeamento NBSP reutilizado desde os lotes 15/16 (um diff
  multi-caractere de substituição/remoção mapeava incorretamente o
  limite final, podendo apagar uma tag XML silenciosamente).
- **Lote 18** (esta rodada, 726qh5): 6 documentos, TJMT/74430633,
  TJRR/568209392, TJRR/568328945, TRF3/42490599, TRF5/349186353
  (Sentença), TRF5/463264301 (Acórdão) -- selecionados deliberadamente
  excluindo os 6 `document_id` do corpo de PR #1574 (lote 17), que
  ainda estava aberta e sem CI reportado no momento da seleção, para
  evitar repetir a classe de risco 12. `document_count` 138->144
  (medido contra `origin/main` antes de #1574 mesclar; os deltas dos
  dois lotes são independentes e aditivos assim que ambos estiverem em
  `main`), teto val/test 21/21->22/22. TJRR/568328945 sofreu um novo
  defeito de transcrição (classe de risco 15, nova): o subagente omitiu
  11 espaços ASCII isolados ao redor de quebras de linha em branco
  (mismatch de fidelidade verbatim de 6368 vs 6385 caracteres) --
  corrigido com uma técnica de diff-e-remapeamento generalizada
  (`docs/planning/evidence/segmenter-djen-sample-batch18-fix-missing-spaces.py`),
  diferente da correção pontual de NBSP porque aqui os caracteres
  foram genuinamente deletados, não substituídos. Nove overrides
  declarados para pares sem cue de fechamento, todos verificados
  contra o texto-fonte bruto: `custas` x4, `honorarios` x3,
  `relatorio` x3 (duas correspondências exatas ao padrão já
  documentado no próprio guideline -- "waiver clause is not a closing
  cue"), `capitulo_merito` x1 (fronteira ambígua causada por um
  precedente citado verbatim embutido na fundamentação de
  TRF3/42490599, que já tinha suas próprias seções I-RELATÓRIO/II-VOTO/
  III-ACÓRDÃO corretamente deixadas sem tag, per Regras 2/3). Um scan
  ao vivo encontrou o pool de TJMG zerado (0 elegíveis abaixo do piso
  de 2500 caracteres), juntando-se a TRF6/TJSC. `uv run ruff
  check`/`format --check` limpos, `uv run pytest -q
  tests/segmenter_dataset` 100% verde.
