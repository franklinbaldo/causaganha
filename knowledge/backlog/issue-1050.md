...
- **Lote 9** (PR #1557, aberta por uma sessão concorrente, ainda não
  mesclada quando o lote 10 rodou): 6 documentos (TJBA, TJMG, TJRS,
  TJSE, TRF2, TJCE). Não refletido no `document_count` deste arquivo
  nem no lote 10 abaixo -- confirmar estado real antes de assumir que
  já mesclou.
- **Lote 10** (esta rodada, imy2ed): 2 documentos visando `preliminar`
  (TJBA/574460089, TJRN/72797727), ambos em tribunais já representados
  com apenas 1 documento cada. `document_count` 109->111, teto de
  val/test 16/16->17/17. Uma primeira seleção (TJRN/72798564,
  TJBA/574460090) foi ingerida e **revertida** ainda nesta rodada --
  ver classe de risco 7 abaixo: os dois candidatos já tinham sido
  ingeridos por um lote anterior sob o mesmo `(tribunal, id_documento)`,
  e a checagem de deduplicação inicial comparou o hash errado, deixando
  passar. Nenhum commit/push referenciou os candidatos errados; a
  correção aconteceu inteiramente antes do primeiro `git add`.
...
documentos totais; estamos em 111 (mais o lote 9 quando mesclar --
recheque ao vivo antes de confiar neste número, dado o ritmo de
rodadas concorrentes neste mesmo dia). A mineração por diversidade de
tribunal está praticamente esgotada (restam apenas STM, TJAC, TJAM,
TJAP, TJPE, TJSP, TRF1 sem candidato usável) -- os lotes 6-10 confirmam
que o caminho daqui em diante é escolher mais candidatos não usados em
tribunais já representados, não perseguir tribunais novos.

**Sete classes de risco/defeito já mapeadas para o próximo lote:**
