...
## Lote 25 (rodada Wisk 20260920T094145Z)

Continuidade direta da linhagem #1050 apos o lote 24 mesclar (PR #1590,
com correcao pos-review em #1591/#1592/#1593). document_count entrava
nesta rodada em 184; piso RFC 0012 Sec 5 item 4 (>=30 val, >=30 test)
seguia inalcancavel mesmo a 100% de adjudicacao (teto 28/28), confirmado
ao vivo por `scripts/segmenter_governance_status.py` antes de qualquer
trabalho.

**Selecao de candidatos.** Ao contrario dos lotes anteriores (que
escaneavam so os tribunais de menor `store_count`), esta rodada escaneou
TODOS os 28 tribunais com pool `data/segmenter_samples/*.jsonl` restante
(exceto TJRO/TRF2, dominados pelo pipeline JURIS, e TRF4, ja reconfirmado
inutilizavel pela classe de risco 16), aplicando o limpador HTML real do
lote 3 (nao `html.unescape()`) e um piso de 2500 caracteres. Seguindo a
licao de processo do lote 24 (o near-duplicate TJBA/574460088 escapou por
comparar candidatos so entre si), a verificacao de near-duplicate desta
rodada comparou cada candidato contra o corpus INTEIRO de 184 documentos
ja no store (`difflib.SequenceMatcher.ratio()`, limiar 0.90), nao so
contra os outros candidatos do proprio lote.

Resultado do scan: TJAC/TJAP/TJPE/TJSP/TRF1 sem nenhum candidato acima do
piso de tamanho (pools vazios nesses arquivos para Sentenca/Acordao).
STM (nunca usado nesta linhagem) tinha exatamente 1 candidato elegivel,
mas com 130784 caracteres -- cerca de 6x o maior documento ja ingerido
com sucesso em qualquer lote anterior (~21000 caracteres). Dado o risco
real de degradacao de atencao ou truncamento na reproducao verbatim
character-a-character por um unico subagente nesse tamanho (nenhuma
tecnica de chunking existe hoje para preservar a exigencia de reproducao
integra do documento), este candidato foi descartado para uma tecnica
futura em vez de arriscar o lote inteiro numa unica anotacao de alto
risco. TJSC/TRF6/TJMG/TJRN/TJRS/TJSE/TJBA/TJMS/TST/TJMA confirmados com 0
candidatos elegiveis (pool inteiro ja absorvido por lotes anteriores,
mesmo esgotamento ja documentado para TJBA desde o lote 24). O scan
completo encontrou 152 candidatos elegiveis distribuidos por
TJCE(6)/TJMT(12)/TJRJ(19)/TJTO(27)/TRF3(14)/TRF5(19)/TJES(4)/TJGO(5)/
TJPA(29)/TJPB(6)/TJPI(1)/TJRR(9).

**Selecao final:** um documento de cada tribunal do tier `store_count=5`
empatado (TJCE/363676235, TJMT/74432048, TJRJ/488920434, TJTO/285641901,
TRF3/42491442, TRF5/463264368 -- elevando cada um para `store_count=6`,
mesmo padrao do lote 24), mais o unico candidato remanescente de
TJPI (`store_count=6`, `22537155`, elevando para 7). Mistura de tipo
mantida (2 Acordaos: TJRJ, TRF5; 5 Sentencas), tamanhos entre 2552 e
14124 caracteres -- dentro da faixa ja comprovada com sucesso.

**Anotacao.** 7 subagentes independentes, um por documento, usando o
prompt canonico Technique 1 (`data/segmenter_splits/technique1_annotation_prompt.md`)
com hint de tipo de documento (Sentenca/Acordao) apropriado a cada um.
Cada subagente verificou sua propria reconstrucao verbatim (strip de tags
== texto-fonte, byte a byte) antes de finalizar.

**Overrides `--allowed-unmatched-overrides` (6 declarados, todos
reverificados contra o texto-fonte bruto antes de declarar):**

- **TJPI/22537155 e TJTO/285641901** (`capitulo_merito`): "DECIDO."/
  "Fundamento e decido." fluem direto para um `dispositivo_abertura` ja
  tageado ("Ante o exposto,"/"Ex positis,") sem cabecalho "DISPOSITIVO"
  intermediario -- mesmo padrao ja sancionado no lote 24 (TJGO/543517919,
  TJMA/42725100).
- **TRF3/42491442** (`relatorio`, `custas`, `honorarios`): "I. Relatorio"
  segue direto para "II. Fundamentos" sem "E o relatorio." ou equivalente;
  "Custas na forma da lei." e "Sem honorarios advocaticios." sao clausulas
  isoladas sem fechamento distinto, seguidas por "Publique-se."
  (encerramento) -- mesmo padrao ja sancionado no lote 18.
- **TRF5/463264368** (`relatorio`, `ementa`): "RELATORIO Dispensado."
  e uma clausula de dispensa padrao (guideline: dispensa nao e cue de
  fechamento); o documento tem um segundo bloco "EMENTA:" com conteudo
  real mais adiante que e seguido por "VOTO" (nao por um segundo
  RELATORIO), sem fechamento distinto para essa ementa.
- **TJRJ/488920434** (`ementa`): formato "capa+ementa-estruturada" --
  "EMENTA" seguida diretamente por secoes numeradas (I. CASO EM EXAME /
  II. QUESTAO EM DISCUSSAO / III. RAZOES DE DECIDIR / IV. DISPOSITIVO),
  sem RELATORIO ou VOTO separado. O guideline ja documenta esse formato
  para exports do TJRO; esta rodada confirma o mesmo padrao estrutural
  tambem em TJRJ.
- **TJMT/74432048** (`custas`, `honorarios`): clausula unica combinada
  "Sem custas processuais e honorarios advocaticios, conforme
  inteligencia dos artigos 54 e 55 da Lei no. 9.099/95." -- mesmo padrao
  ja sancionado no lote 18.

**Verificacao pos-ingestao.** `scripts/segmenter_semantic_audit.py`
sinalizou 1 achado novo: `doc_2f952744ab8c9e0bafb66cd01a9f4e2d`
(TRF3/42491442), `fundamentacao_legal_collapsed` (1 tag, mas "art."
aparece >3 vezes no texto bruto). Verificacao ao vivo confirmou falso
positivo: as demais mencoes de "art." (art. 1.021 CPC/2015, art. 557,
art. 195,I) vivem inteiramente dentro de um bloco de precedentes de
terceiros citados verbatim sob "Confiram-se os precedentes:" (excertos
STJ/TRF3, incluindo um export com marcadores EMEN:/DTPB: tipicos de base
de jurisprudencia) -- nao sao a fundamentacao PROPRIA deste documento,
mesma logica ja aplicada ao TJMA/42725100 no lote 24. Registrado como
decisao (`decision-dismiss-trf3-quoted-precedent-art-finding`) e o
oitavo `doc_id` da allowlist documentada em
`tests/segmenter_dataset/test_segmenter_audit_scripts.py::test_real_store_has_at_most_the_one_known_collapsed_false_positive`
(o teste falhava antes desta atualizacao, exatamente como o comentario
do teste avisa: "a new real omission was introduced and needs the same
triage" -- triagem feita, nenhuma omissao real, allowlist estendida com
razao documentada).

`uv run ruff check`/`format --check` limpos. `uv run pytest -q
tests/segmenter_dataset` 100% verde apos a atualizacao da allowlist.
document_count 184->191, annotation_count 237->244, teto de val/test
28/28->29/29 (`scripts/segmenter_governance_status.py`, confirmado ao
vivo; lote train-only, sem segunda anotacao independente).

Ainda por continuar: falta aproximadamente document_count>=~200 para o
piso RFC 0012 Sec 5 item 4 se tornar alcancavel -- cerca de 1-2 lotes
deste tamanho no ritmo atual. O pool de tribunais ja representados esta
ficando raso (152 candidatos elegiveis nesta rodada, concentrados em
TJPA/TJTO/TJRJ/TRF5/TRF3); STM permanece como o unico tribunal
inteiramente novo com um candidato disponivel, mas bloqueado por tamanho
(130784 caracteres) ate existir uma tecnica de chunking ou um subagente
comprovadamente capaz de reproduzir documentos dessa escala com
fidelidade verbatim. Uma rodada futura deve reescanear ao vivo (o pool
muda pouco entre rodadas, mas os overrides ja declarados nesta rodada
nao devem ser reusados sem reverificacao) e considerar se vale a pena
...
2 documentos reais, novos e não sobrepostos com qualquer lote anterior:
TJES/577054715 (Sentença, 3824 chars) e TJGO/543518267 (Sentença, 4195
chars após `html.unescape()`). Snapshot antes deste lote: 193
documentos, 246 anotações, teto de val/test 29/29.

Esta rodada não teve acesso a uma ferramenta de spawn de subagentes
(`Task`/`Agent`) no ambiente em que rodou -- diferente de toda rodada
anterior documentada neste arquivo, que usava um subagente por
documento seguindo `technique1_annotation_prompt.md`. A anotação dos
dois documentos foi produzida diretamente por esta sessão, seguindo a
mesma metodologia do prompt canônico (ler o guideline, enumerar
categorias esperadas antes de taguear, produzir a reprodução tagueada
via inserção de tags por offset -- a mesma técnica de "não retipar
nada" já estabelecida desde o lote 9 -- e então verificar o rascunho),
e então **verificada de forma independente** com
`segmenter_dataset.store._text_element_to_labels` real (o mesmo
parser mecânico que o script de ingestão usa) antes de qualquer
ingestão, não apenas um autorrelato. Registrado aqui explicitamente
como uma divergência de processo forçada pela ferramenta disponível
neste ambiente, não uma escolha deliberada de pular a rigidez usual --
uma rodada futura com acesso a `Task` deve voltar a usar um subagente
por documento.

Um scan ao vivo de `data/segmenter_samples/*.jsonl` (Sentença/Acórdão
apenas, >=2500 chars, deduplicado por `(tribunal, id)` derivado do
nome do arquivo contra os `source_uri` `djen_sample_technique1` já no
store) encontrou o tier de menor `store_count` não-exaurido em 6,
empatado entre TJES/TJGO/TJPB/TJMT/TJPA/TJRJ/TJTO/TRF3/TRF5.
TRF6/TJMG/TJSC/TJRN/TJRS/TJSE/TJMS/TST reconfirmados exauridos ou
abaixo do piso; TRF4 reconfirmado inutilizável pela classe de risco 16
já documentada; TJBA (574460088) e TJMA (42728925) reconfirmados como
as mesmas quase-duplicatas já rejeitadas pelo lote 26 (0.980/0.968 de
ratio contra documentos já no store) -- não reselecionados. TJES e
TJGO foram escolhidos por estarem no tier de menor `store_count`
empatado (6) com candidatos genuinamente disponíveis, e ambos
confirmados livres de quase-duplicata por um `difflib.SequenceMatcher.ratio()`
ao vivo contra o corpus inteiro de 193 documentos (razão máxima 0.114
para TJES, contra `djen_sample_technique1:batch1:TJES:577039281`
já ingerido; razão máxima 0.054 para TJGO, contra um documento TJRO
não relacionado).

TJGO/543518267 teve o mesmo defeito recorrente já documentado nos
lotes 12/16/18/22 para esse tribunal: `texto_limpo` continha
entidades HTML brutas não decodificadas (`&nbsp;`/`&Aacute;`/etc, sem
markup embutido) -- resolvido com `html.unescape()` antes de anotar
(5111 chars brutos -> 4195 chars limpos). O texto resultante tinha 25
ocorrências de NBSP genuíno (U+00A0) preservadas verbatim (não
substituídas por espaço comum), confirmadas presentes byte-a-byte na
reconstrução mecânica.

TJES/577054715 tem o formato duplo "projeto de sentença" (juiz leigo)
+ homologação (juiz togado) do Juizado Especial, já visto no corpus em
`doc_a33e4d32f1892f5c9e91177ba1f78a34` (TJMT/74428001, lote 18):
seguindo o mesmo precedente já estabelecido nesse documento,
`dispositivo_abertura`/`resultado` foram tageados na ruling do JUIZ
LEIGO ("Isso posto"/"JULGO EXTINTO"), não na homologação do juiz
togado ("Com tais fundamentos"/"homologo"), que fica deliberadamente
sem tag (Regra 2 do guideline: apenas um `dispositivo_abertura`
operativo por decisão). Pela mesma lógica, uma segunda menção restated
de custas/honorários no trecho de encerramento (já homologado) foi
deliberadamente deixada sem tag -- só a menção operativa (a do juiz
leigo) foi tageada.

Três overrides `--allowed-unmatched-overrides` foram declarados, todos
verificados contra o texto-fonte bruto antes de declarar:
`relatorio` em ambos os documentos (heading/cue de abertura presente
-- "Trata-se de" em TJES, "I – RELATÓRIO" em TJGO -- mas sem
"É o relatório." ou equivalente em nenhum dos dois; TJES também não
tem heading `capitulo_merito` separado, TJGO tem um heading
`capitulo_merito` completo e fechado normalmente, "II – FUNDAMENTAÇÃO"
até "III – DISPOSITIVO"), e `custas`+`honorarios` em TJES ("Deixo de
condenar em custas e honorários, nesta fase, por determinação legal
(artigo 55, Lei 9.099/95)" -- cláusula única combinada sem cue de
fechamento distinto para nenhum dos dois, mesmo padrão já sancionado
nos lotes 12/18/19/20/22/24/25/26).

Fidelidade verbatim de ambos reverificada de forma independente via
`segmenter_dataset.store._text_element_to_labels` real (não
autorrelato) antes da ingestão -- 16 labels brutos em TJES e 17 em
TJGO, reduzidos a 15 cada após a ingestão real por
`EXCLUDED_CATEGORIES={'ref_normativa'}` (comportamento de produção já
existente e intencional, não um defeito desta rodada: TJES tinha 1
`ref_normativa` e TJGO tinha 2, ambos corretamente excluídos do espaço
de rótulos treinável -- a mesma classe de "falso positivo" da
allowlist de `fundamentacao_legal_collapsed` já documentada para
outros documentos do corpus). `git status --short data/segmenter`
confirmou exatamente 2 novos `documents/*.xml` e 2 novos
`annotations/<id>/`, sem write no-op silencioso.

`scripts/segmenter_semantic_audit.py` sinalizou zero achados novos: os
7 `doc_id`s `_collapsed` da allowlist de
`tests/segmenter_dataset/test_segmenter_audit_scripts.py` são
exatamente os mesmos de antes, e nenhum dos dois novos documentos
aparece em qualquer achado (incluindo os achados não-`_collapsed`
como `long_anchor`/`dispositivo_inside_voto`, já pré-existentes e
não relacionados a este lote). `uv run ruff check`/`format --check`
limpos; `uv run pytest -q tests/segmenter_dataset` 100% verde (exit
code 0).

`document_count` 193->195, `annotation_count` 246->248, teto de
val/test inalterado em 29/29 (lote train-only, sem segunda anotação
independente; 2 documentos não foi suficiente para cruzar o piso RFC
0012 Sec 5 item 4 de >=30/>=30 -- um próximo lote deste tamanho ou
maior ainda é necessário).

Nenhum defeito de código de produção encontrado nesta rodada. A única
incerteza genuína não totalmente eliminável: a escolha editorial de
qual `dispositivo_abertura`/`resultado` é o "operativo" no formato
projeto+homologação do TJES foi resolvida por analogia direta ao
precedente já existente no corpus (TJMT/74428001), não por uma regra
explícita do guideline v7 para esse caso específico -- se uma revisão
futura (Codex ou humana) discordar dessa leitura, é o mesmo tipo de
achado editorial (não um bug mecânico) já visto nos lotes 23-25.
