...
blocking_reason: "Not blocked. Fourteen real batches now proven through scripts/ingest_djen_sample_technique1_batch.py, run under alternating report mechanisms (legacy AgentRun scaffold and the current Wisk runtime) without needing a production-code change for most of them: batch1 (0iuk22) 7 docs/7 tribunals; batch2 (jyqinl) 6 docs/6 tribunals; batch3 (uyx7xc) 7 docs/7 tribunals; batch4 (mg2tp1) 5 docs/3 tribunals; batch5 (la7bsl) 7 docs (4 TJMS + 2 TJPA + 1 TJPI), first round to widen the candidate-length floor to 2500 chars and surface TJMS as a 25th tribunal; batch6 (Wisk round, PR #1549) 3 docs across TRF3/TJCE/TJMT (already-represented tribunals), targeting the 'preliminar' cue; batch7 (round zrek2s, PR #1553) 6 docs across TJRJ/TJGO/TJTO/TJPB/TJMA/TJRR, also targeting 'preliminar', and fixed a real production bug (see risk class 5 below); batch8 (round 83kr8s, PR #1552) 8 docs across TRF5/TJMT/TJRR/TJPA/TRF3/TJRJ/TJPB/TJES; batch9 (round hv2ep2, PR #1557) 6 docs across TJBA/TJMG/TJRS/TJSE/TRF2/TJCE (all already-represented tribunals, chosen for lowest store document-count rather than rare-category cue; hit a real concurrency collision of its own, see risk class 8 below); batch10 (round imy2ed, PR #1559) 2 docs targeting 'preliminar' (TJBA/574460089, TJRN/72797727) -- an initial selection (TJRN/72798564, TJBA/574460090) was reverted mid-round after discovering both were already-ingested duplicates (see risk class 9 below); batch11 (Wisk round, PR #1562) 2 docs, TJRN/72796443 and TJMA/42728353, both already-represented tribunals at the lowest store_count tier (2 each) -- no unused 'preliminar'-cue candidate remained anywhere in an already-represented tribunal after a fresh full scan, so this batch followed batch9's volume strategy instead; hit a new process defect, see risk class 10 below; batch12 (round 5lvbii, PR #1563) 2 docs, TJES/577054686 and TJGO/543562390, tied for the lowest non-singleton store_count (2 each) with a 'preliminar' cue each; batch13 (round 96cgqx, PR #1565) 2 docs, TJSE/578949084 (Acordao) and TJRS/458637070 (Sentenca), both the ONLY remaining eligible candidate for their respective tribunal at the time; batch14 (this Wisk round, PR #1567, resolved on top of an outdated pre-batch13 base) picked the SAME 5 lowest-store_count candidates batch13 had already seen (TJPI/22443810, TJRS/458637070, TJSE/578949084, TRF5/349055692, TST/237077355) -- a genuine concurrent-session collision discovered while resolving this PR's merge conflict against main, not a tooling bug. TJRS collided on the same deterministic document id (redundant annotation dropped, no new document); TJSE collided on two distinct document ids for the same real document with slightly different transcriptions (this batch's copy dropped entirely, keeping batch13's merged instance). Net new contribution: 3 documents -- TJPI/22443810, TRF5/349055692, TST/237077355. A sixth candidate (TJSC/587254906) was selected, annotated and ingested by batch14, then reverted: it turned out to already exist in the store from batch4 (PR #1545), because data/segmenter_samples/tjsc_acordao.jsonl's own info.tribunal field is blank for every record in that file, silently breaking the (tribunal, id) dedup check against the store's real tribunal value (see risk class 11 below). document_count moved 61->68->74->81->86->93->96->102->109->115->117->119->121->123->126, val/test ceiling 17/17 (batch9, 115 docs) ->18/18 (117-123 docs) ->19/19 (126 docs, batch14) -- recheck scripts/segmenter_governance_status.py live rather than trusting any cached number in this file, including this one. Still far below RFC 0012 Sec 5 item 4's >=30/>=30 floor (needs roughly 200 total documents)."
unblock_condition: "Already unblocked, fourteen rounds of proof the ingestion path scales without code changes (one real production bug found and fixed along the way by batch7 -- see risk class 5). A future round should keep running batches through scripts/ingest_djen_sample_technique1_batch.py against the remaining pool in data/segmenter_samples/*.jsonl. Always verify document_count/tribunal distribution LIVE via scripts/segmenter_governance_status.py and SegmenterDatasetStore.list_documents() before selecting a batch's candidates -- this backlog file's own numbers have repeatedly lagged real state between concurrent rounds (multiple independent sessions picked up the same next_move within the same day for batches 6-14, including a full 5-of-5-candidate overlap between batch13 and batch14); do not trust last_verified_run_id's snapshot without a live re-check, and expect another concurrent session to be working this same issue at any given moment. Before spawning an annotation subagent for a candidate, dedupe it correctly per risk class 9 below (content_hash(text) + (tribunal, id_documento)-vs-source_uri, NOT any externally-sourced hash field) -- AND per risk class 11, derive the tribunal side of that pair from the jsonl FILENAME, not from the candidate record's own info.tribunal field, since at least one file (tjsc_acordao.jsonl) has that field blank for every record. After a dry-run ingest, always verify each returned document_id against the real store's documents/<id>.xml before writing for real (git status --short data/segmenter after a REAL ingest is the reliable tell: a candidate that produces only a new annotations/<id>/ann_*.xml with no matching new documents/<id>.xml was already in the store -- revert that annotation file, it adds no independence value if its annotator_config matches the pre-existing one). If a PR's merge conflict against main reveals the SAME (tribunal, id_documento) pair ingested independently by both sides under DIFFERENT document ids (not merely a redundant annotation on one shared id), that means two distinct transcriptions of one real document -- keep only the side already merged into main and drop the other's document+annotation pair entirely, renumbering the batch to reflect only its net-new contribution (this is what batch14 did for TJSE). Tribunal-diversity mining is exhausted (only STM, TJAC, TJAM, TJAP, TJPE, TJSP, TRF1 remain without a usable candidate) -- the path forward is picking more unused Sentenca/Acordao candidates from already-represented tribunals by volume (lowest store_count first), not chasing new tribunals or a specific rare category. When a start/end pair has no closing cue in the source text (a recurring, expected shape, not a bug -- see risk classes 4/6), verify against the raw source text before declaring a --allowed-unmatched-overrides reason, same as every prior batch that hit this. A source jsonl with raw HTML markup (<b>/<table>/<br>/etc, not just entities) needs the batch3 HTML-to-text cleaner (docs/planning/evidence/segmenter-djen-sample-batch3-clean-html.py) BEFORE handing the text to an annotation subagent, not just html.unescape(). IMPORTANT for whichever mechanism picks this up next: knowledge/agent-runs/index.md and .claude/hourly-loop.md declare the legacy AgentRun mechanism deprecated in favor of the Wisk runtime (.wisk/knowledge/) for the CausaGanha hourly loop. This BacklogItem type is not itself named in the deprecation list (only AgentRun/AgentReading/AgentGoal/AgentDecision/AgentEvidence/AgentCheck are), so it is kept updated here regardless of which mechanism a future round uses, but check .wisk/knowledge/ too before assuming this file alone is current. Budget for eleven known defect/risk classes before trusting a batch's first pass -- see the numbered list below, including risk class 5 (a real production fix, already merged) and risk classes 7-11 (candidate-selection/concurrency/tooling process bugs, not production-code bugs)."
last_verified_run_id: "wisk:20260916T202659Z-do-the-best-useful-work-available-in-this-reposi"
...
documentos totais; estamos em 123 apos mesclar o lote 13 (recheque
...
TJSP, TRF1 sem candidato usável, reconfirmado pelo lote 12) -- os lotes
6-13 confirmam que o caminho daqui em diante é escolher mais candidatos
não usados em tribunais já representados por volume (menor
`store_count` primeiro): TJPI/TJRJ/TJMG/TRF5/TRF2/TJTO estão no menor
nível (2 cada) apos o lote 13. A categoria rara `preliminar` **não**
está esgotada nesse universo -- o lote 13 encontrou dezenas de
candidatos não usados nos próprios tribunais do menor nível (ver acima);
o "esgotamento" relatado pelo lote 11 era escopo apenas dos tribunais
checados naquele momento.
...
Esta rodada partiu do commit anterior à mesclagem do lote 13 real
(rodada 96cgqx, PR #1565) e escolheu, de forma independente, o mesmo
conjunto de 5 candidatos de menor `store_count` que aquela rodada viu:
TST/237077355, TJPI/22443810, TJRS/458637070, TJSE/578949084,
TRF5/349055692 -- uma colisão de concorrência genuína (classe de risco 8/9),
não um bug de ferramenta, descoberta apenas ao resolver o conflito de merge
do PR #1567 contra o `main` já avançado.

TJRS/458637070 recaiu no mesmo `document_id` determinístico
(`doc_cfa06dbce6009c921f4f66a5126c2056`) em ambas as anotações
independentes, então o merge produziu apenas uma segunda anotação
redundante sobre um documento já existente (nenhum documento novo, nenhuma
inflação de `document_count`) -- essa anotação duplicada foi descartada
(mesmo `annotator_config`, sem valor de independência para IAA, mesmo
padrão já estabelecido para o TJSC deste lote). TJSE/578949084 é um caso
diferente: cada anotação independente gerou um `document_id` distinto e
uma transcrição ligeiramente diferente (esta rodada divergiu da fonte em
um caractere de pontuação) -- manter os dois teria contado o mesmo
documento real duas vezes sob hashes diferentes, então o par
documento+anotação desta rodada para TJSE foi descartado por completo,
mantendo apenas a instância já mesclada do lote 13 (prefixo de hash
`c002c5d5`).

Contribuição líquida desta rodada: 3 documentos novos --
TJPI/22443810, TRF5/349055692, TST/237077355 -- todos em tribunais já
representados, escolhidos por `store_count` mais baixo com um candidato
limpo disponível. `document_count` 123->126 (base 123 já inclui o lote 13
real), teto de val/test 18/18->19/19. Um sexto candidato (TJSC/587254906)
foi selecionado, anotado e ingerido nesta mesma rodada, depois revertido
-- ver classe de risco 11 acima. Dois candidatos (TJSC, TST) tinham markup
HTML bruto na fonte (não só entidades) e precisaram do limpador
`docs/planning/evidence/segmenter-djen-sample-batch3-clean-html.py` antes
da anotação; dois candidatos (TJSC, TJRS) tiveram NBSP mid-documento
substituído por espaço comum pelo subagente na primeira tentativa,
corrigido por reanotação com instrução explícita; o candidato TJSE
descartado tinha caracteres de controle ASCII (0x13/0x1c/0x1d) usados como
aspas improvisadas, substituídos por aspas ASCII comuns antes da anotação
(mesma mitigação da classe 7, mas o documento em si não sobreviveu ao
dedup pós-merge). `uv run pytest -q tests/segmenter_dataset/` verde,
incluindo o teste de regressão `test_real_store_reflects_batch13_corpus_growth`
(inalterado, cobre o lote 13 real) e o novo
`test_real_store_reflects_batch14_corpus_growth` (cobre a contribuição
líquida deste lote); audit semântico sem novos achados para os 3
documentos mantidos.
