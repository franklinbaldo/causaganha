...
evidence_ids:
  ...
  - "2026-09-08-exciting-mccarthy-b4t8pv-evidence-pr-1338-merged"
check_ids:
  ...
  - "2026-09-08-exciting-mccarthy-b4t8pv-check-okf-parser-close"
result_state: "merged"
result_summary: "Fixed a real, currently-live public-metric bug: totals.qmd and tribunal_coverage.qmd's 'pending' bucket exact-matched djen_status = 'available', silently excluding djen_status='confirmed' rows -- a real value src/djen_backup/probe.py writes into the canonical sync-manifest.parquet for pairs DJEN has confirmed available but not yet uploaded, which src/djen_backup/drain.py actively prioritises for upload. Any such row was counted in `total` but in none of uploaded/pending/absent/unknown, undercounting the public /stats page's per-tribunal pending column (web/src/pages/stats.astro:185) for any tribunal with a probe-confirmed-but-unuploaded pair. Widened both filters to `djen_status IN ('available', 'confirmed')`, the exact pattern render_manifest_parquet.py's own _print_merge_stats already uses. Wrote 2 RED-then-GREEN tests (new manifest_parquet_confirmed_pending fixture), confirmed RED against the unfixed queries (pending=0 for a row that should count), then GREEN after the fix. tests/test_render_queries.py: 39/39 (up from 37). Full uv run pytest -q green with zero failures once this report was finalized. ruff check clean; ruff format --check needed one auto-fix (a long line in the new test fixture), then clean. okf-parser check knowledge --relational-schema okf.schema.sql: conformant, 0 diagnostics throughout the round (798 -> 814 concepts across 3 checks). PR #1338 opened, all 3 workflow runs on the head commit (OKF knowledge, CI/test.yml, Product Surface Visual Capture) completed with conclusion=success, mergeable_state 'clean', zero reviews pending -- squash-merged via mcp__github__merge_pull_request as commit efa595a onto main."
next_move: "This round's work is fully merged (PR #1338) -- the issue/PR queue is exhausted again. A future round should re-read issues/PRs fresh and, if still empty, continue the Explore-subagent-driven codebase survey that found this round's bug (it also independently re-checked and ruled out the four `except Exception` sites, the coverageInsights.ts dead code, and the download_zip() 403-typing gap as not worth this round's slot). Leads still available for a future round: (a) the except-Exception/BLE001 policy gap in src/djen_backup/archive.py:264, src/causaganha/analysis/llm_analyzer.py:387,499, src/causaganha/consolidate/cli.py:160 (needs an architectural decision -- narrow to specific exception types vs. document as a deliberate resilience boundary -- before it's TDD-able); (b) web/src/queries/README.md's stale optional-contracts list missing datajud_totals/datajud_classes (one-line docs fix, no behavior change); (c) consolidation_status.qmd's prose ('have Parquets on IA') still doesn't match what ia_status='uploaded' actually measures (DJEN zip upload, not Parquet consolidation) -- this round confirmed the dataset has zero frontend consumers (grepped web/src/pages and web/src/components), so it's a docs-only fix, not user-facing, lower priority than a reachable bug. A new lead surfaced but not investigated this round: court_reliability.qmd and tribunal_calendar.qmd have no 'available'-exact filter so are unaffected by this round's specific bug, but neither has any test coverage for a djen_status='confirmed' row either -- worth a quick audit to confirm they truly have no analogous gap."
---
