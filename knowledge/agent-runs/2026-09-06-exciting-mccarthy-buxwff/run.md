...
next_move: "Push this branch, open a PR closing #1219, and drive it to green/mergeable per this session's standing PR-babysitting duty (react to CI, review comments, and the visual-diff expectations #1219 itself calls for). The agents-surface-capture.yml workflow currently only triggers on web/src/pages/agentes.astro / web/src/components/Header.astro / web/src/pages/sitemap.xml.ts and only screenshots /agentes — it does not cover this round's actual changed files (Layout.astro, index.astro) or the home route, so it will not automatically supply the 'captura visual cobre home + navegação' evidence #1219's acceptance criteria ask for; a future round (or CI feedback on the PR) should extend that workflow's paths/capture targets to include the home route and Layout.astro, or a reviewer may ask for it as a review comment on this PR. Future rounds: knowledge/backlog/'s 17 blocked issues remain trusted as of this round's own re-verification; #1219 should no longer appear as open once this PR merges; PR #1220 (from session uwm65t) remains open, clean, and unrelated to this round's work."
---
