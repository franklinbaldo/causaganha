...
On the domain work itself: a concurrent Wisk-driven session (PR #1549, merged first) worked issue #1050 in parallel with this round, landing 3 more documents (TRF3/TJCE/TJMT, targeting the 'preliminar' cue) under the same 'sixth batch' label before either round had merged -- a real naming collision from two uncoordinated sessions picking up the same next_move at once, not a conflict in the actual data (different candidate documents, clean merge, verified live: document_count 104 post-merge = 101 (this round in isolation) + 3 (PR #1549) exactly). knowledge/backlog/issue-1050.md has been relabeled (their batch as Lote 6, this round as Lote 7) to keep the historical count accurate.

245 more unused, in-range (2500-18000 char, Sentenca/Acordao) real candidates remain in data/segmenter_samples/*.jsonl after this batch (258 pre-batch minus the 8 consumed here, not accounting for whatever PR #1549's session may have separately consumed from the same pool -- recount live with the same query rather than trusting this arithmetic). A future round (whichever mechanism it uses) should: (1) keep running batches through scripts/ingest_djen_sample_technique1_batch.py against that remaining pool -- tribunal diversity is confirmed exhausted twice now (la7bsl and this round), so freely pick additional candidates from any already-represented tribunal, prioritizing ones with the deepest remaining pools; (2) before trusting a first-pass mechanical-validation failure on a dangling relatorio/capitulo_merito/custas/honorarios pair, inspect the raw tagged text directly (not the subagent's self-report) -- this round's 6 cases were both known shapes (Juizado Especial 'relatorio dispensado', short custas/honorarios clauses with no closing phrase), so a re-annotation cycle would have been wasted; declare a specific override reason per (candidate, category) instead, per decision-accept-dangling-pairs-as-overrides; (3) after any --allowed-unmatched-overrides usage, double check knowledge/backlog/issue-1050.md actually got the update its own result_summary claims -- this round found and fixed a real gap where la7bsl's PR merged with a result_summary claiming the backlog file was updated when it was not, so verify with grep before trusting a prior round's own narrative; (4) continue tracking scripts/segmenter_governance_status.py's val_ceiling/test_ceiling after each batch -- now at 16/16 (post-merge) against a >=30/>=30 floor, so #1051 (adjudication) remains correctly parked until corpus size closes most of that gap (roughly halfway there by document_count, 104 of ~200). After this round: push, open PR with these before/after numbers, follow CI to green, merge, and record the merge outcome in a closeout commit as every prior round in this lineage has done."
---
