...
async def test_analyzer_initialization(mock_agent):
    ...
    analyzer = DecisionAnalyzer(model="gemini-test")
...
@pytest.mark.asyncio
async def test_analyze_decision_success(mock_agent):
    ...
    assert len(args) == 2
    # assert isinstance(args[1], BinaryContent) # Can import BinaryContent to check type


@pytest.mark.asyncio
async def test_analyze_decision_failure(mock_agent):
...
@pytest.mark.asyncio
async def test_analyze_batch_placeholder(mock_agent):
    """Test batch analysis.
