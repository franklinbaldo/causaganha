...
async def test_repository_get_unanalyzed(memory_db: BaseBackend) -> None:
    ...
    con.raw_sql("""
        ...
        VALUES
        (1, 'proc1', '2024-01-01', 'TJRO', TRUE, 'link1', 'Int', 'Org', 'Txt', 'Doc', 'Cls', 'abc'),
        (2, 'proc2', '2024-01-01', 'TJRO', FALSE, 'link2', 'Int', 'Org', 'Txt', 'Doc', 'Cls', 'abc'),
        (3, 'proc3', '2024-01-01', 'TJRO', FALSE, NULL, 'Int', 'Org', 'Txt', 'Doc', 'Cls', 'abc')
    """)
    ...
    ids = [i.id for i in results]  # Access as object attribute
    assert 2 in ids
    assert 1 not in ids
