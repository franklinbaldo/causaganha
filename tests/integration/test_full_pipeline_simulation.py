...
class TestFullPipelineSimulation:
    ...
    async def test_complete_pipeline_collect_to_score(
        ...
        with patch.object(
            ...
            await run_collection(
                repository, client, start_date="2024-12-15", end_date="2024-12-15", courts=["TJRO"],
            )
        ...
        archived = (
            con.table("intimations").filter(con.table("intimations").ia_url.notnull()).execute()
        )
        ...
        with patch("causaganha.pipeline.analyze.DecisionAnalyzer") as MockAnalyzer:
            ...
            with patch.object(
                ...
                # Run analysis
                await run_analysis(repository, doc_service, analyzer, limit=10)
        ...
        scored_analyses = (
            ...
            .filter(con.table("analysis_results").scored == True)
            .execute()
        )
    ...
    async def test_pipeline_handles_missing_links(
        ...
        result = con.table("intimations").filter(con.table("intimations").id == 2001).execute()
