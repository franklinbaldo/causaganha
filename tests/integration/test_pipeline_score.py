...
async def test_run_scoring(tmp_path: Path) -> None:
    ...
    # Store using AnalysisRepository
    # AnalysisRepository expects AnalysisResult objects usually, but check implementation
    # Wait, the memory says "AnalysisRepository handles store_analysis_results_batch"
    # and "AnalysisRepository.store_analysis_results_batch supports both AnalysisResult objects and dictionaries as input."

    await analysis_repo.store_analysis_results_batch([analysis_data])

    # Run scoring
    lawyer_repo = LawyerRatingRepository(con)
    await run_scoring(analysis_repo, lawyer_repo)
