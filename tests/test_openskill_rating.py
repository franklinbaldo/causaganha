...
@pytest.mark.parametrize("result, expected_ranks", [
    ("win_a", [0, 1]),
    ("win_b", [1, 0]),
    ("draw", [0, 0]),
...
def test_rate_teams_outcomes(default_model: PlackettLuce, result: str, expected_ranks: list[int]):
    ...
    if result == "win_a" or (result == "partial_a" and default_model.tau > 0): # partial_a should behave like win_a if tau > 0
        assert p1_updated.mu > original_p1_mu or (p1_updated.mu == original_p1_mu and p1_updated.sigma < original_p1_sigma) # Winner's mu increases or sigma decreases
        assert p2_updated.mu < original_p2_mu or (p2_updated.mu == original_p2_mu and p2_updated.sigma < original_p2_sigma) # Loser's mu decreases or sigma decreases (uncertainty can decrease)
    elif result == "win_b" or (result == "partial_b" and default_model.tau > 0):
        assert p2_updated.mu > original_p2_mu or (p2_updated.mu == original_p2_mu and p2_updated.sigma < original_p2_sigma)
        assert p1_updated.mu < original_p1_mu or (p1_updated.mu == original_p1_mu and p1_updated.sigma < original_p1_sigma)
    elif result == "draw":
...
def test_example_from_module(default_model: PlackettLuce):
    ...
    player2_ex1 = create_rating(default_model, mu=30.0, sigma=default_model.sigma, name="Player2_ex1")
    player3_ex1 = create_rating(default_model, name="Player3_ex1")

    team_a_ex1 = [player1_ex1, player2_ex1]
    team_b_ex1 = [player3_ex1]

    updated_team_a_ex1, updated_team_b_ex1 = rate_teams(default_model, team_a_ex1, team_b_ex1, 'win_a')
    p1_updated_ex1, p2_updated_ex1 = updated_team_a_ex1
    p3_updated_ex1 = updated_team_b_ex1[0]

    assert p1_updated_ex1.mu > player1_ex1.mu or p1_updated_ex1.sigma < player1_ex1.sigma
    assert p2_updated_ex1.mu > player2_ex1.mu or p2_updated_ex1.sigma < player2_ex1.sigma
    assert p3_updated_ex1.mu < player3_ex1.mu or p3_updated_ex1.sigma < player3_ex1.sigma

    # Test draw
    player1_ex2 = create_rating(default_model, name="Player1_ex2")
    player2_ex2 = create_rating(default_model, mu=30.0, sigma=default_model.sigma, name="Player2_ex2")
    player3_ex2 = create_rating(default_model, name="Player3_ex2")
    team_a_ex2 = [player1_ex2, player2_ex2]
    team_b_ex2 = [player3_ex2]
    updated_team_a_draw, updated_team_b_draw = rate_teams(default_model, team_a_ex2, team_b_ex2, 'draw')
    ...
    player2_ex3 = create_rating(default_model, mu=30.0, sigma=default_model.sigma, name="Player2_ex3")
    player3_ex3 = create_rating(default_model, name="Player3_ex3")
    team_a_ex3 = [player1_ex3, player2_ex3]
    team_b_ex3 = [player3_ex3]
    updated_team_a_partial, updated_team_b_partial = rate_teams(default_model, team_a_ex3, team_b_ex3, 'partial_a', partial_play_tau=0.7)
