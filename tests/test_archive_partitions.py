...
def test_verified_catalog_discovery_refuses_global_search_when_manifest_empty(monkeypatch):
    from scripts import generate_catalog as catalog

    monkeypatch.setattr(catalog, "get_items_from_sync_manifest", lambda: [])
    global_search = Mock(return_value=["djen-unrelated-tribunal-2026"])
    monkeypatch.setattr(catalog, "list_ia_items", global_search)

    items, _existing_manifest, _completed_items = catalog.discover_catalog_items(
        full=True, verified_inventory=True
    )
