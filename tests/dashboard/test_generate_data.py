...
def test_fetch_kilo_ready_count_all_success(mock_urlopen: MagicMock) -> None:
    ...
    mock_urlopen.side_effect = [
        MagicMock(__enter__=lambda _: mock_prs_response, __exit__=lambda *_: None),
        MagicMock(__enter__=lambda _: mock_checks_response, __exit__=lambda *_: None),
    ]
...
@patch("urllib.request.urlopen")
def test_fetch_kilo_ready_count_with_failures(mock_urlopen: MagicMock) -> None:
...
@patch("urllib.request.urlopen")
def test_fetch_kilo_ready_count_pending_checks(mock_urlopen: MagicMock) -> None:
...
@patch("urllib.request.urlopen")
def test_fetch_kilo_ready_count_empty_checks(mock_urlopen: MagicMock) -> None:
    ...
    mock_checks_response = MagicMock()
    mock_checks_response.read.return_value = json.dumps({"check_runs": []}).encode("utf-8")
...
@patch("urllib.request.urlopen")
def test_fetch_kilo_ready_count_kilo_success(mock_urlopen: MagicMock) -> None:
