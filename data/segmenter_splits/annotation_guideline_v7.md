...
| `ementa` | "EMENTA:" | Last line before RELATÓRIO. **TJRO "capa+ementa-estruturada" exports** (Ementa: followed directly by numbered sections — I. CASO EM EXAME / II. QUESTÃO(ÕES) EM DISCUSSÃO / III. RAZÕES DE DECIDIR / IV. DISPOSITIVO E TESE — with no separate RELATÓRIO or VOTO section) have no real closing cue at all: those four numbered sections *are* the published ementa content, not a section after it. Leave `ementa_fim` unmatched (extends to EOD) for this format — don't fabricate a closing tag at some later point in the document (e.g. inside a trailing "Jurisprudência relevante citada" block) just because text continues after the last numbered section |
