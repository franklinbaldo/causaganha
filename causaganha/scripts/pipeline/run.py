...
def main():
    ...
    if args.job in ("all", "collect"):
        cmd = ["uv", "run", "python", f"{scripts_dir}/collect.py", "--date", target_date, "--proxy-url", args.proxy_url]
        res = execute_step("collect", cmd, repo_root)
        state = PipelineState(results=state.results + (res,), files_added=res.outputs.get("files_added") == "true")
    ...
    if args.job in ("all", "consolidate"):
        ...
        state = PipelineState(results=state.results + (res,), files_added=state.files_added or res.outputs.get("files_added") == "true")
        if not res.success: sys.exit(1)
    ...
    if args.job == "all" or state.files_added:
        execute_step("catalog", ["uv", "run", "python", f"{repo_root}/scripts/generate_catalog.py", "--upload"], repo_root)
        execute_step("dashboard", ["uv", "run", "python", f"{repo_root}/scripts/generate_dashboard_cache.py"], repo_root)
