...
def update_elo(rating_a: float, rating_b: float, score_a: float, k_factor: int = DEFAULT_K_FACTOR) -> tuple[float, float]:
    """
...
if __name__ == '__main__':
    ...
    logging.info(f"Inputs: rating_a={r_a1}, rating_b={r_b1}, score_a=1.0, k_factor={DEFAULT_K_FACTOR}")
    ...
    logging.info(f"Rating change for A: {new_r_a1_win - r_a1:+.2f}, for B: {new_r_b1_win - r_b1:+.2f}")

    # Scenario 1.2: Draw (score_a = 0.5)
    logging.info(f"\nInputs: rating_a={r_a1}, rating_b={r_b1}, score_a=0.5, k_factor={DEFAULT_K_FACTOR}")
    e_a1_draw = expected_score(r_a1, r_b1) # Expected score doesn't change based on outcome
    ...
    logging.info(f"Rating change for A: {new_r_a1_draw - r_a1:+.2f}, for B: {new_r_b1_draw - r_b1:+.2f}")
    ...
    logging.info(f"Output: A loses -> New Ratings: A={new_r_a1_loss}, B={new_r_b1_loss}")
    logging.info(f"Rating change for A: {new_r_a1_loss - r_a1:+.2f}, for B: {new_r_b1_loss - r_b1:+.2f}")
    ...
    logging.info(f"Output: A (1600) wins against B (1400) -> New Ratings: A={new_r_a2_win}, B={new_r_b2_win}")
    logging.info(f"Rating change for A: {new_r_a2_win - r_a2:+.2f}, for B: {new_r_b2_win - r_b2:+.2f}")
    ...
    logging.info(f"Output: A (1600) loses against B (1400) -> New Ratings: A={new_r_a2_loss}, B={new_r_b2_loss}")
    logging.info(f"Rating change for A: {new_r_a2_loss - r_a2:+.2f}, for B: {new_r_b2_loss - r_b2:+.2f}")

    # Example 3: Different K-factor
    r_a3, r_b3 = 1500.0, 1500.0
    k_custom = 32
    logging.info(f"\n--- SCENARIO 3: Custom K-Factor (K={k_custom}) ---")
    logging.info(f"Inputs: rating_a={r_a3}, rating_b={r_b3}, score_a=1.0, k_factor={k_custom}")
    ...
    logging.info(f"Output: A wins (K={k_custom}) -> New Ratings: A={new_r_a3_win}, B={new_r_b3_win}")
    logging.info(f"Rating change for A: {new_r_a3_win - r_a3:+.2f}, for B: {new_r_b3_win - r_b3:+.2f}")

    # Test ValueError for score_a
    logging.info(f"\n--- SCENARIO 4: Invalid Score Input ---")
    try:
        logging.info(f"Inputs: rating_a=1500, rating_b=1500, score_a=1.5, k_factor={DEFAULT_K_FACTOR}")
