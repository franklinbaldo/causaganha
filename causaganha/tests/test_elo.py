...
class TestEloCalculations(unittest.TestCase):
    ...
    def test_expected_score_higher_rating(self):
        ...
        self.assertAlmostEqual(
            elo.expected_score(1900, 1500), 1 / (1 + math.pow(10, -1)), places=4
        )
        self.assertTrue(
            elo.expected_score(1900, 1500) > 0.9
        )  # A is much more likely to win

    def test_expected_score_lower_rating(self):
        # Significantly lower rating for A
        # E_A = 1 / (1 + 10^((1900 - 1500) / 400)) = 1 / (1 + 10^(1)) = 1 / 11 = 0.0909...
        self.assertAlmostEqual(
            elo.expected_score(1500, 1900), 1 / (1 + math.pow(10, 1)), places=4
        )
        self.assertTrue(
            elo.expected_score(1500, 1900) < 0.1
        )  # A is much less likely to win
    ...
    def test_update_elo_equal_ratings_a_wins(self):
        ...
        self.assertAlmostEqual(
            new_r_a + new_r_b, r_a + r_b
        )  # Sum of Elo should be constant
    ...
    def test_update_elo_different_ratings_higher_wins(self):
        ...
        self.assertAlmostEqual(
            new_r_a + new_r_b, r_a + r_b
        )  # Sum of Elo should still be very close to constant
    ...
    def test_update_elo_different_ratings_lower_wins_upset(self):
        ...
        change = k * (
            1.0 - e_a
        )  # Should be positive and larger than k*0.5 (big gain for upset)
