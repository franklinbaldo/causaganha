...
import shutil # For test setup/teardown
import pandas as pd # For pipeline update tests
import datetime # For pipeline update tests (partidas data_partida)
...
class TestPipelineArgParsingAndExecution(unittest.TestCase):
    ...
    def test_run_command_orchestration(self, mock_fetch, MockGeminiExtractor, mock_update_cmd):
        ...
        mock_extractor_instance.extract_and_save_json.assert_called_once_with(
            ...
            output_json_dir=Path('causaganha/data/json/'), # Default for run command's extract step
            dry_run=False
        )
    ...
    def test_verbose_flag_sets_debug_level_basicConfig(self, mock_basic_config):
        ...
        with patch.object(pipeline, '_update_elo_ratings_logic', MagicMock()): # Mock out the actual logic
    ...
    def test_verbose_logging_capture_for_update(self, mock_get_logger):
        ...
        self.assertTrue(
            any("Update command called with args" in str(arg_call) for arg_call in mock_logger_instance.debug.call_args_list)
        )
...
class TestPipelineUpdateCommand(unittest.TestCase):
    def setUp(self):
        ...
        self.sample_decision_1 = {
            "numero_processo": "0000001-01.2023.8.22.0001", "tipo_decisao": "sentença",
            "partes": {"requerente": ["Adv A Teste (OAB/UF 111)"], "requerido": ["Adv B Teste (OAB/UF 222)"]},
            "advogados": {"requerente": ["Adv A Teste (OAB/UF 111)"], "requerido": ["Adv B Teste (OAB/UF 222)"]},
            "resultado": "procedente", "data_decisao": "2023-01-01"
        }
        self.sample_decision_2 = {
            "numero_processo": "0000002-02.2023.8.22.0001", "tipo_decisao": "sentença",
            "partes": {"requerente": ["Adv C Teste (OAB/UF 333)"], "requerido": ["Adv A Teste (OAB/UF 111)"]},
            "advogados": {"requerente": ["Adv C Teste (OAB/UF 333)"], "requerido": ["Adv A Teste (OAB/UF 111)"]},
            "resultado": "improcedente", "data_decisao": "2023-01-02"
        }
        ...
        pd.DataFrame(columns=['advogado_id', 'rating', 'total_partidas']).set_index('advogado_id').to_csv(self.ratings_csv_path)
    ...
    def test_update_command_valid_decisions(self, mock_read_csv, mock_to_csv, mock_shutil_move):
        ...
        logging.basicConfig(level=logging.DEBUG, stream=sys.stderr) # Ensure logs are visible for test debug
        args = pipeline.argparse.Namespace(dry_run=False, verbose=True) # Simulate parsed args
    ...
    def test_update_command_all_decisions_invalid(self, mock_read_csv, mock_to_csv, mock_shutil_move, mock_validate):
        ...
        self.assertEqual(mock_to_csv.call_count, 1) # ratings.csv (potentially empty or initial)
        ...
        self.assertTrue(mock_validate.call_count >= 2) # Called for decisions in both files
