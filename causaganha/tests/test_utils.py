...
class TestNormalizeLawyerName(unittest.TestCase):
    def test_title_removal(self):
        ...
        self.assertEqual(normalize_lawyer_name("DR.  QUX"), "qux") # Extra space after DR.
        self.assertEqual(normalize_lawyer_name("dr. foo"), "foo") # Lowercase title

    def test_accent_normalization(self):
        self.assertEqual(normalize_lawyer_name("João Álves da Silva"), "joao alves da silva")
        self.assertEqual(normalize_lawyer_name("José Élio Çइंटيا"), "jose elio cइंटيا") # Corrected expected output
    ...
    def test_combined_normalization(self):
        self.assertEqual(normalize_lawyer_name("  DRA.  MARÍA ÇLARA  NÚÑEZ  "), "maria clara nunez")
        self.assertEqual(normalize_lawyer_name("Dr. Antônio de Oliveira e Silva"), "antonio de oliveira e silva")

    def test_oab_string_preservation(self):
        self.assertEqual(normalize_lawyer_name("Dr. José (OAB/SP 123)"), "jose (oab/sp 123)")
        self.assertEqual(normalize_lawyer_name("Maria Silva OAB/RJ12345"), "maria silva oab/rj12345")
...
class TestValidateDecision(unittest.TestCase):
    def setUp(self):
        self.valid_decision = {
            ...
            "partes": {"requerente": ["Fulano de Tal"], "requerido": "Cicrano Indústria Ltda"},
    ...
    def test_missing_numero_processo(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with("Validation failed: 'numero_processo' is missing or empty.")
    ...
    def test_bad_numero_processo_format(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with(f"Validation failed: 'numero_processo' ({invalid['numero_processo']}) does not match pattern [\\d.-]{{15,25}}.")
    ...
    def test_numero_processo_not_string(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with(f"Validation failed: 'numero_processo' is not a string (got {type(invalid['numero_processo'])}). Value: {invalid['numero_processo']}")
    ...
    def test_missing_partes(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with(f"Validation failed: 'partes' is missing or not a dictionary (got <class 'NoneType'>).")
    ...
    def test_partes_not_dict(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with(f"Validation failed: 'partes' is missing or not a dictionary (got {type(invalid['partes'])}).")
    ...
    def test_missing_requerente(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with("Validation failed: 'partes.requerente' is missing or empty.")
    ...
    def test_empty_requerente_string(self): # Assuming string is allowed based on current validate_decision
        invalid = self.valid_decision.copy()
        invalid["partes"]["requerente"] = ""
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
    ...
    def test_requerente_wrong_type(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with(f"Validation failed: 'partes.requerente' is not a list or string (got {type(invalid['partes']['requerente'])}).")
    ...
    def test_missing_requerido(self): # Similar tests for 'requerido'
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with("Validation failed: 'partes.requerido' is missing or empty.")
    ...
    def test_missing_resultado(self):
        ...
        with patch.object(logging.getLogger('causaganha.legalelo.utils'), 'warning') as mock_log:
            self.assertFalse(validate_decision(invalid))
            mock_log.assert_called_with("Validation failed: 'resultado' is missing or empty.")
