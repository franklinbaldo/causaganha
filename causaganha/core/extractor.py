...
try:
    ...
    genai = None
    logging.warning("Module google.generativeai could not be imported. Ensure it is installed correctly. GeminiExtractor will use dummy responses if API key is also missing.")
...
class GeminiExtractor:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        if genai and self.api_key:
            try:
                ...
                logging.info("GeminiExtractor initialized: google.generativeai configured with API key.")
                self.gemini_configured = True
            except Exception as e:
                logging.error(f"GeminiExtractor: Failed to configure google.generativeai: {e}")
                self.gemini_configured = False
        elif genai and not self.api_key:
            logging.warning("GeminiExtractor initialized: google.generativeai imported, but API key missing. Real API calls will be skipped.")
            self.gemini_configured = False
        else: # genai is None
            logging.info("GeminiExtractor initialized: google.generativeai not imported. Real API calls will be skipped.")
            self.gemini_configured = False
    ...
    def extract_and_save_json(self, pdf_path: str | pathlib.Path, output_json_dir: str | pathlib.Path) -> pathlib.Path | None:
        ...
        if not self.gemini_configured:
            logging.warning(f"Skipping real Gemini API call for {pdf_path.name} (Gemini not configured or API key missing). Returning dummy data structure.")
            ...
            final_extracted_data = {
                ...
                "advogados": {"requerente": ["N/A (OAB/UF)"], "requerido": ["N/A (OAB/UF)"]},
...
Não inclua markdown (```json ... ```) na sua resposta."""
                ...
                logging.info(f"Generating content for {pdf_path.name} using 'gemini-1.5-flash'...")
                ...
                try:
                    ...
                    if isinstance(extracted_data_from_api, list):
                        ...
                        extracted_data_from_api["extraction_timestamp"] = datetime.datetime.utcnow().isoformat() + "Z"
                        final_extracted_data = extracted_data_from_api
                    else:
                        logging.error(f"Gemini response for {pdf_path.name} was not a JSON dict or list: {type(extracted_data_from_api)}")
                        final_extracted_data = None # Or some error structure

                except json.JSONDecodeError as je:
                    logging.error(f"Failed to parse JSON response from Gemini for {pdf_path.name}: {je}")
                    logging.debug(f"Gemini raw response text for {pdf_path.name}: {response.text[:500]}...") # Log snippet
                    final_extracted_data = None

            except Exception as e:
                logging.error(f"Error during Gemini API call or processing for {pdf_path.name}: {e}")
                final_extracted_data = None # Ensure it's None if any step failed
            finally:
                if uploaded_file_object:
                    try:
                        logging.info(f"Deleting uploaded file {uploaded_file_object.name} from Gemini.")
                        genai.delete_file(uploaded_file_object.name)
                        logging.info(f"Successfully deleted file {uploaded_file_object.name}.")
                    except Exception as e_del:
                        logging.error(f"Error deleting file {uploaded_file_object.name} from Gemini: {e_del}")

        if final_extracted_data is None:
            logging.warning(f"No data extracted or error occurred for {pdf_path.name}. JSON file will not be saved.")
            return None
        ...
        if isinstance(final_extracted_data, dict) and "numero_processo" in final_extracted_data:
            process_number_to_use = final_extracted_data["numero_processo"]
        elif isinstance(final_extracted_data, dict) and "decisions" in final_extracted_data and \
             isinstance(final_extracted_data["decisions"], list) and len(final_extracted_data["decisions"]) > 0 and \
             "numero_processo" in final_extracted_data["decisions"][0]:
...
def main():
    parser = argparse.ArgumentParser(description="Extract structured data from a PDF document using Gemini.")
    parser.add_argument(
        "--pdf_file",
        type=pathlib.Path,
        required=True,
    ...
    if saved_path:
        ...
        logging.warning(f"Extraction process failed or produced no output for {args.pdf_file}.")
...
if __name__ == '__main__':
    ...
    cli_test_pdf = dummy_pdf_dir / "cli_test_doc_for_extractor.pdf" # Different name to avoid conflict
    if not cli_test_pdf.exists():
        try:
            ...
            with open(cli_test_pdf, "wb") as f:
                writer.write(f)
            logging.info(f"Created dummy PDF (blank page) for CLI testing: {cli_test_pdf}")
        except ImportError:
            with open(cli_test_pdf, 'w') as f:
                f.write("This is a dummy PDF content for CLI testing (google-generativeai not available or text-only dummy).")
            logging.info(f"Created basic dummy PDF (text file) for CLI testing: {cli_test_pdf}")
        except Exception as e:
            logging.error(f"Could not create dummy PDF: {e}. Falling back to simple text file.")
            with open(cli_test_pdf, 'w') as f:
