...
describe('rendered query contracts', () => {
  it('renders fixture-backed query outputs that match every frontend schema', () => {
    execFileSync('uv', ['run', 'python', '../scripts/render_contract_fixture.py', fixtureRoot], {
      cwd: process.cwd(),
      stdio: 'inherit',
    });

    const manifest = JSON.parse(readFileSync(join(fixtureRoot, 'query-contracts.json'), 'utf8')) as RenderManifest;
    ...
    for (const query of manifest.contracts.filter((contract) => !contract.optional)) {
      expect(existsSync(join(fixtureRoot, 'web/public', query.output)), `${query.name} must be emitted`).toBe(true);
    }
    ...
    for (const output of emittedOutputs) {
      ...
      expect(result.success, `${frontend!.name}: ${result.success ? '' : result.error.message}`).toBe(true);
    }
