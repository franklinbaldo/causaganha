...
type Config struct {
	...
	Path                  string            `toml:"-"`
	DiarioDeJusticaBearer string            `toml:"de_justica_bearer"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("DJEN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "djen-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("DIARIO_DE_JUSTICA_BEARER"); v != "" {
		cfg.DiarioDeJusticaBearer = v
		cfg.AuthSource = "env:DIARIO_DE_JUSTICA_BEARER"
	}
	...
	if cfg.AuthSource == "" && cfg.DiarioDeJusticaBearer != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("DJEN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.DiarioDeJusticaBearer
	if token == "" {
		return ""
	}
	if c.DiarioDeJusticaBearer == "" {
		return ""
	}
	return token
}
