...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".djen-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DJEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DJEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "djen-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.djen-pp-cli/feedback.jsonl.
When ` + "`DJEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DJEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  djen-pp-cli feedback list
  djen-pp-cli feedback list --limit 5
  djen-pp-cli feedback list --json`,
