...
Exit codes & warnings:
  ...
  djen-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  djen-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  djen-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  djen-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  djen-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/djen-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "caderno":
		return db.UpsertCaderno(data)
	case "comunicacao":
		return db.UpsertComunicacao(data)
	case "certidao":
		return db.UpsertCertidao(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"comunicacao-tribunal": "/api/v1/comunicacao/tribunal",
	}
