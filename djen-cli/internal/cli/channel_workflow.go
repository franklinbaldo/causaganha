...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  djen-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  djen-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"comunicacao", "comunicacao-tribunal"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  djen-pp-cli workflow status

  # Show status as JSON
  djen-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("djen-pp-cli")
			}
