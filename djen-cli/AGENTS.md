# Djen Printed CLI Agent Guide

This directory is a generated `djen-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
djen-pp-cli doctor --json
djen-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
djen-pp-cli which "<capability>" --json
djen-pp-cli <command> --help
```
...
```bash
djen-pp-cli <command> --help
djen-pp-cli <command> --dry-run --agent
```
