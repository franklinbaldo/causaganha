...
- **Atraso publicação→arquivo** (tempo entre DJEN confirmar disponibilidade
  e o ZIP estar no Internet Archive): meta declarada de 24h, alinhada à
  cadência diária real do pipeline (`consolidate-parquet.yml` roda às
  07:00 UTC), mas ainda não tem alarme automatizado — exigiria consultar o
  manifesto por linhas `djen_raw` disponível há mais de 24h sem
  `ia_status=uploaded`. Não implementado neste PR; o campo `pending_real`
  já existente em `site-status.json` é o dado bruto para isso, faltando só
  o limiar e o alarme.
