...
### 🗃️ **Core Archive Module (`src/archive_db.py`)**
...
### 🖥️ **CLI Integration (`src/pipeline.py`)**
- **New `archive` command**: `uv run python src/pipeline.py archive`
...
uv run python src/pipeline archive

# Monthly archive for specific date
uv run python src/pipeline archive --date 2025-06-26 --archive-type monthly

# Dry run to test configuration
uv run python src/pipeline archive --dry-run

# Direct archive module usage
uv run python src/archive_db.py --date 2025-06-26 --archive-type weekly --verbose
```
