...
| TM-02 | Alta | Relay roubado/abusado vira WAF-bypass contra tribunais, aceita método/rota indevida ou encaminha headers sensíveis. | Egress do relay é HTTPS-only, métodos fechados, host/rota fechados, headers mínimos, redirects manuais e budgets de tamanho/quota. | `djen_proxy.go` (fixed-host) agora só encaminha GET sob `/api/` — `/login`, `/swagger/` e `/comunicacao` solto foram removidos por falta de uso real (`deployment/djen_proxy_test.go`, CI job `djen-proxy`). Python relay usa token em comparação constante, allowlist de host e não segue redirects, mas ainda aceita método amplo e não faz stripping de `Authorization`/`Cookie`; CF relay já é HTTPS + GET/HEAD/POST e faz stripping mais forte, mas com a mesma lacuna de `Authorization`/`Cookie`. | Testes rejeitam HTTP, métodos mutáveis, host/rota fora da política, headers sensíveis e body/resposta acima do limite; mantêm requests oficiais necessários. Feito para `djen_proxy.go`; pendente para os dois relays. | [#1609](https://github.com/franklinbaldo/causaganha/issues/1609) |
