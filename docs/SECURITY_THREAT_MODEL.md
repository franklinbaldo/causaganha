...
| TM-03 | Alta se houver alteração canônica; média para egress isolado | Manifest/index envenenado controla URL entregue a DuckDB/httpfs ou browser. | Toda URL derivada de autoridade remota passa por uma única política: HTTPS, host canônico, path/extensão esperados, quantidade limitada e quoting central. | `arquivo_ia_url` vindo de `indice_processual.parquet` passa por `service._validate_artifact_url` (Python) e `validateArtifactUrl` (TypeScript, `web/src/lib/processoCnj.ts`) antes de qualquer `read_parquet`/lista SQL — mesma política nos dois runtimes (HTTPS, host `archive.org`, prefixo `/download/`, sufixo `.parquet`, sem query/fragment/aspas). Superfície distinta, mesma classe de ameaça: `causaganha.decisoes.published._juris_url` monta a URL do parquet JURIS a partir de `mes_ano`/`tipo` lidos do manifesto JURIS publicado (buscado ao vivo de `archive.org` por `juris_archive.read_manifest_text`) — como `urllib.parse.quote()` preserva `/` por padrão, um `mes_ano` malicioso sobrevivia intacto até a URL final consumida por `read_parquet([...])` em `causaganha.decisoes.search` (reproduzido ao vivo: `mes_ano="2024-01/../../secret-item"` produzia uma URL com `../../` sobrevivente). `tjro_juris.manifest.ManifestJuris.load_text` agora rejeita (`ManifestFormatError`) qualquer `mes_ano` fora da forma canônica `YYYY-MM` (`_MES_ANO_PATTERN`) no parse do manifesto, antes que `_juris_url` o veja. | Fixtures `file://`, `http://`, host estranho, quotes, fragments/query e paths inesperados falham fechado antes de DuckDB/fetch; Python e TS usam política equivalente. Feito — `TestValidateArtifactUrl`/`test_poisoned_manifest_url_degrades_source_instead_of_crashing` (Python) e testes equivalentes em `processoCnj.test.ts`. `mes_ano` fora de `YYYY-MM` (incluindo `/../`) falha fechado no parse do manifesto JURIS — feito em `tests/tjro_juris/test_juris_manifest.py::test_load_text_rejects_malformed_mes_ano` (8 casos parametrizados) e `tests/causaganha/decisoes/test_published.py::test_juris_discovery_rejects_manifest_with_path_traversal_mes_ano`. | [#1610](https://github.com/franklinbaldo/causaganha/issues/1610) |
| TM-04 | Alta | Manifesto formalmente válido aponta para geração/tribunal/período errado, produzindo omissão ou atribuição incorreta sem precisar de SSRF. | Cada transição preserva identidade verificável: source, retrieval time, raw artifact, hash, normalization version, generation id, tribunal/período, schema fingerprint, row count e derived artifact. | DataJud bundles já usam generation + SHA-256 + size; manifests e canários possuem vários checks de coerência. Para o dossiê unificado (`indice_processual.parquet`), `service._validar_tribunal_coerente`/`_tribunal_da_url` cruzam o `tribunal` declarado por linha contra o tribunal que o próprio `arquivo_ia_url` nomeia no item IA (`djen-{tribunal}-{ano}`, `datajud-{tribunal}`) — as duas fontes particionadas por tribunal; `juris`/`stj` usam item fixo, sem o que cruzar. Além disso, todo export djen já grava `causaganha.schema_version`/`causaganha.item_id` no rodapé KV_METADATA do próprio Parquet (`schema_registry.kv_metadata_for_export`, ativo desde a v3.0.0) — `service._validar_metadata_djen`/`_validar_metadata_djen_urls` agora leem esse rodapé via `parquet_kv_metadata()` (leitura de rodapé, não do arquivo inteiro) antes de compor `read_parquet`, e descartam (aviso, não exceção) qualquer artefato djen cujo `schema_version` seja desconhecido ou cujo `item_id` divirja do item_id que a própria URL do índice nomeia — a identidade que o *artefato* declara sobre si mesmo, não só a URL/manifesto. `arquivo_ia_url` também é selecionado com `tribunal` em `web/src/lib/processoCnj.ts::buildIndiceSql` (paridade de linha com o harness #1107), e agora as duas checagens têm paridade do lado Web: `fonteUrls`/`validarTribunalCoerente`/`tribunalDaUrl` cruzam o `tribunal` do índice contra o tribunal que `arquivo_ia_url` nomeia (mesma política, mesmo bypass para URL/fonte não particionada), e `validarMetadataDjenUrls`/`validarMetadataDjen`/`itemIdDaUrl` leem o rodapé Parquet de cada artefato djen individualmente via `parquet_kv_metadata('<url>')` antes de compor `read_parquet`/`buildDjenSql`, descartando (aviso, não exceção) qualquer um cujo `schema_version` seja desconhecido ou cujo `item_id` divirja da URL do índice — só disparado quando a URL já tem a forma de item djen (`itemIdDaUrl` não-nulo), para não arriscar uma leitura de rede numa fixture/URL sem reivindicação a verificar. `KNOWN_DJEN_SCHEMA_VERSIONS` em `processoCnj.ts` duplica manualmente as chaves de `SCHEMA_REGISTRY` (não há import cross-language) — mesmo risco de divergência já aceito para a política de URL de artefato (Python `_validate_artifact_url` vs TS `validateArtifactUrl`). `juris`/`stj`/`datajud` não emitiam esse KV_METADATA — `tjro_juris.service._rows_to_parquet` agora grava `causaganha.schema_version`/`causaganha.item_id` (namespace `causaganha.*` compartilhado com o registry djen, `JURIS_SCHEMA_VERSION="1.0.0"`) via `pyarrow.Table.replace_schema_metadata` antes de cada export mensal, com `item_id` derivado do item IA (`tjro-juris-{ano}`) que `tjro_juris.archive` já usa para upload — lado de escrita fechado e comprovado legível tanto por `pq.read_schema(...).metadata` quanto por `parquet_kv_metadata()` (mesma função DuckDB que o lado de leitura djen usa), mas o lado de leitura (validar o rodapé juris antes de compor `read_parquet` em `causaganha.decisoes.published`/`_juris_url`) ainda não existe — gap real, restante, documentado. `stj`/`datajud` continuam sem emitir esse rodapé. Hash de conteúdo completo e row-count continuam fora de alcance: exigiriam baixar o arquivo inteiro, contradizendo o uso de httpfs para consulta seletiva — decisão registrada, não uma lacuna esquecida. | Fixture com tribunal incompatível com o artefato falha antes da composição — feito para djen/datajud em `tests/causaganha/processos/test_service.py` (`TestTribunalCoerenteComUrl`, `test_poisoned_manifest_tribunal_degrades_source_instead_of_trusting`) e no espelho TS `web/src/lib/processoCnj.test.ts` (`tribunalDaUrl / validarTribunalCoerente`, `degrades djen to absent + aviso when the index tribunal disagrees with the URL it names`). Fixture com rodapé Parquet (`item_id`/`schema_version`) incoerente com a URL do índice falha e degrada com aviso — feito em Python (`TestMetadataDjenCoerente`, `test_djen_artifact_footer_item_id_mismatch_degrades_source_instead_of_trusting`, `test_djen_artifact_footer_item_id_coerente_is_accepted`) e no espelho TS (`itemIdDaUrl / validarMetadataDjen`, `validarMetadataDjenUrls`, `degrades djen to absent + aviso when the artefact's own Parquet footer disagrees with the URL it is served at`). Footer juris com `causaganha.schema_version`/`causaganha.item_id` corretos, provado por `test_rows_to_parquet_embeds_schema_version_and_item_id_in_footer` e `test_rows_to_parquet_footer_metadata_is_readable_via_duckdb_parquet_kv_metadata` (`tests/tjro_juris/test_juris_service.py`) — feito (lado de escrita). Pendente: lado de leitura para juris (validar antes de `read_parquet` em `_juris_url`); KV_METADATA equivalente para stj/datajud; hash/row-count de conteúdo completo (decisão: fora de alcance sem uma revisão maior de como httpfs consulta esses artefatos). | [#1610](https://github.com/franklinbaldo/causaganha/issues/1610) |
...
| TM-10 | Crítica se comprometer job/build com autoridade de publicação | Resolução livre de dependências/base image permite build não reprodutível ou pacote compatível malicioso. | Mesmo commit resolve para inputs verificáveis e runtime possui privilégio mínimo. | Ações são majoritariamente SHA-pinned; JS possui lockfiles. `uv.lock` é commitado e CI (`.github/actions/setup`) e o build de produção (`deploy-mcp.yml`) rodam `uv sync --frozen` — resolução diverge de `pyproject.toml` falha o build em vez de re-resolver silenciosamente. `deployment/mcp/Dockerfile` instala via `uv sync --frozen` a partir desse lock (não mais `pip install .` livre), pina `python:3.12-slim` por digest e roda como usuário não-root (`mcp`, uid 10001). O job `supply-chain` (`.github/workflows/test.yml`, roda em toda PR) exporta o mesmo conjunto `--no-dev` que o Dockerfile instala, escaneia com `pip-audit` (falha fechado em qualquer vulnerabilidade conhecida) e publica o SBOM CycloneDX resultante como artefato de build. `safety` foi removido das dev dependencies: era um scanner concorrente nunca invocado por nenhum script/CI, e sua própria dependência transitiva `nltk` carregava uma vulnerabilidade sem fix disponível (PYSEC-2026-3740) — removê-lo eliminou a vulnerabilidade junto com a ferramenta morta, em vez de silenciá-la. Escopo aceito e documentado: dependências de ferramentas de desenvolvimento que nunca embarcam na imagem (`mkdocs-material`/`pymdown-extensions`, travado abaixo da versão corrigida pelo próprio pin de `marimo==0.23.14`, `pymdown-extensions<11`) ficam fora do gate de scan — bump de `marimo` é uma mudança comportamental não relacionada ao notebook tooling, fora do escopo desta fatia. | `tests/deployment/test_mcp_deployment.py` cobre lock commitado, `uv sync --frozen` no Dockerfile e em CI, digest pin e `USER` não-root antes do `CMD`. `tests/deployment/test_supply_chain_scan.py` cobre a existência do job `supply-chain`, o escopo `--no-dev` do scan, o gate fechado (sem `|| true`/`continue-on-error`) e a publicação do SBOM. | [#1614](https://github.com/franklinbaldo/causaganha/issues/1614) |
| TM-11 | Média; alta apenas se host executar ação sensível de forma confiável | Prompt injection indireta em publicação/decisão influencia o LLM consumidor a usar tools externas ao MCP. | Texto judicial é evidência não confiável e não instrucional de forma machine-readable; ARQUIVO/ESTADO/TEOR não se misturam. | `publicacoes_buscar`, `decisoes_buscar` e `processo_consultar` marcam cada item de texto judicial com `tipo_conteudo="untrusted_legal_text"` (`causaganha_mcp.evidence.UNTRUSTED_LEGAL_TEXT`, `src/causaganha_mcp/tools/publicacoes.py`, `decisoes.py` e `processo.py` — `DocumentoResult.resumo`, `StjAcordaoResult.tese`/`ementa`) — texto passa verbatim, nunca sanitizado/promovido a campo operacional. | Schema preserva marker de confiança em todas as três tools (`tests/causaganha_mcp/test_untrusted_evidence_marker.py`); fixture “ignore instruções anteriores” permanece no campo de teor e nunca vira campo operacional/next action. | Fechado — [#1616](https://github.com/franklinbaldo/causaganha/issues/1616) |
