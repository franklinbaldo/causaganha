...
| TM-10 | Crítica se comprometer job/build com autoridade de publicação | Resolução livre de dependências/base image permite build não reprodutível ou pacote compatível malicioso. | Mesmo commit resolve para inputs verificáveis e runtime possui privilégio mínimo. | Ações são majoritariamente SHA-pinned; JS possui lockfiles. `uv.lock` agora é commitado e CI (`.github/actions/setup`) e o build de produção (`deploy-mcp.yml`) rodam `uv sync --frozen` — resolução diverge de `pyproject.toml` falha o build em vez de re-resolver silenciosamente. `deployment/mcp/Dockerfile` instala via `uv sync --frozen` a partir desse lock (não mais `pip install .` livre), pina `python:3.12-slim` por digest e roda como usuário não-root (`mcp`, uid 10001). Pendente: SBOM do artefato/imagem e scanner (`pip-audit`/scan de imagem) integrados a um gate de CI — exige escolher ferramenta, fora do escopo desta fatia. | `tests/deployment/test_mcp_deployment.py` cobre lock commitado, `uv sync --frozen` no Dockerfile e em CI, digest pin e `USER` não-root antes do `CMD`. Pendente: gate de SBOM/scan. | [#1614](https://github.com/franklinbaldo/causaganha/issues/1614) |
