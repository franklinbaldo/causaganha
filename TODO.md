...
- [x] **End-to-End Testing**
    ...
    - [x] Update E2E test to verify V2 flow (migrated to `tests/v2/e2e/test_full_lifecycle.py`).
- [x] **Documentation**
    - [x] Update `README.md` with new architecture diagrams.
    - [x] Generate API docs with `mkdocs`.

## Phase 5: V2 Integration & Migration (✅ Completed)
- [x] **Implement V2 Archive Pipeline**
    - [x] Add archive queries to `src/causaganha/v2/storage/queries.py`
    - [x] Create `src/causaganha/v2/pipeline/archive.py`
- [x] **Migrate CLI to V2**
    - [x] Update `src/causaganha/cli.py` to use V2 pipeline modules
    - [x] Verify `collect` command
    - [x] Verify `archive` command
    - [x] Verify `analyze` command
    - [x] Verify `score` command
- [x] **Verify E2E**
    - [x] Ensure `tests/v2/e2e/test_full_lifecycle.py` passes with V2 components
