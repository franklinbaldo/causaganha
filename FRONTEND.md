...
Zod is used to validate all external data at the boundary — API responses, URL query parameters, JSON files. There are two canonical sources, for two different kinds of boundary:

- **`web/src/lib/data/contracts.ts`** — hand-written schemas for the `.qmd` query contracts (see "Manifest query contracts" in the root `CLAUDE.md`), one per dataset under `web/public/data/`. `web/src/lib/data/index.ts`'s `loadContract()` is the single call site that parses against them (`schema.safeParse(json)`), both at Astro build time and from the browser.
- **`web/src/lib/djen-zod.gen.ts`** — schemas generated by [orval](https://orval.dev/) directly from the DJEN OpenAPI spec (`djen.yml`). `djenClient.ts` parses the live DJEN response against `GetApiV1ComunicacaoResponse.safeParse(body)` before anything else touches it.

`web/src/lib/djen.ts` itself no longer contains any Zod schema — its own file docstring documents why: it used to hand-write ~200 lines of Zod schemas that duplicated `djen.yml`, and those were replaced by the generated `djen-zod.gen.ts` types plus a set of hand-written normalization functions (`normalizeDateString`, `normalizeMeio`, `normalizePolo`, `normalizeExternalUrl`) that handle the specific messiness the spec doesn't capture — inconsistent date formats, mixed enum spellings, `camelCase`/`snake_case` field drift. Read `djen.ts`'s header comment before touching DJEN response handling; don't assume it works like `contracts.ts`.
...
Use `.parse()` when failure should throw (API responses where bad data is a bug — this is what `loadContract()` does at build time, per its own doc comment: "fail-loud"). Use `.safeParse()` when you want to handle validation failure gracefully in the UI (what `loadContract()` does client-side, and what `djenClient.ts` does for the live DJEN response).

### Coerce messy data by hand when Zod's own coercion isn't enough

This project does not currently use `z.preprocess`. Where API data needs real normalization — mixed date formats, inconsistent enum spellings, `null`-vs-missing fields — `djen.ts` uses small, explicitly-typed functions instead of a Zod transform, so the coercion logic is unit-testable on its own:

```ts
// web/src/lib/djen.ts
export function normalizeMeio(value: unknown): DjenMeio | undefined {
  if (value === null || value === undefined || value === '') return undefined;
  const raw = normalizeLabel(value); // trim, lowercase, strip accents
  if (raw === 'd' || raw === 'diario' || raw === 'eletronico') return 'D';
  if (raw === 'e' || raw === 'edital') return 'E';
  return undefined;
}
```

`contracts.ts`'s own schemas lean on plain Zod methods instead (`.nullable()` on every column the SQL can produce as `NULL`) rather than `z.preprocess`, since query-contract JSON comes from `scripts/render_queries.py`'s own typed SQL output, not a third-party API — there's much less to coerce.
...
- **Accessibility.** There is no dedicated accessibility guidelines document. The only automated guard is `web/src/components/svelteA11y.contract.test.ts`, which compiles every `.svelte` file and fails on any Svelte-compiler `a11y_*` warning (e.g. a noninteractive element with a `tabindex`). It catches compiler-detectable issues only — it does not check color contrast, focus order, or screen-reader semantics. Do not introduce new accessibility regressions, and do not assume a written guidelines doc exists to consult.
