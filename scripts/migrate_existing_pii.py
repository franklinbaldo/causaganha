...
from src.utils import normalize_lawyer_name # For consistency if needed, though PiiManager handles normalization internally based on type
...
def migrate_ratings_table(db: CausaGanhaDB, pii_manager: PiiManager, dry_run: bool = False):
    ...
    try:
        ...
        current_ratings_data = db.conn.execute("SELECT advogado_id, mu, sigma, total_partidas FROM ratings").fetchall()
        ...
        for i, (original_adv_id, mu, sigma, total_partidas) in enumerate(current_ratings_data):
            if original_adv_id in processed_original_ids: # Should not happen if adv_id is PK, but good check
                logger.warning(f"Duplicate original_adv_id '{original_adv_id}' found in ratings. Skipping subsequent entry.")
                continue
            ...
            if (i + 1) % 100 == 0:
                logger.info(f"Processed {i+1}/{len(current_ratings_data)} ratings for UUID mapping...")


        if not dry_run:
            logger.info("Deleting all existing records from 'ratings' table before re-inserting with UUIDs...")
            db.conn.execute("DELETE FROM ratings")
            logger.info("All records deleted from 'ratings'.")

            logger.info(f"Re-inserting {len(new_ratings_to_insert)} records into 'ratings' with UUIDs as advogado_id...")
            ...
            logger.info(f"DRY-RUN: Would delete {len(current_ratings_data)} records from 'ratings'.")
            logger.info(f"DRY-RUN: Would re-insert {len(new_ratings_to_insert)} records into 'ratings' with UUIDs.")
...
def migrate_decisoes_table(db: CausaGanhaDB, pii_manager: PiiManager, dry_run: bool = False):
    ...
    try:
        decision_ids = [row[0] for row in db.conn.execute("SELECT id FROM decisoes ORDER BY id").fetchall()]
        ...
        for i, record_id in enumerate(decision_ids):
            row_data_tuple = db.conn.execute("SELECT * FROM decisoes WHERE id = ?", (record_id,)).fetchone()
            ...
            # 1. numero_processo
            if row.get('numero_processo') and not 'uuid' in str(row.get('numero_processo')).lower(): # Avoid re-processing if it looks like a UUID already
                original_np = str(row['numero_processo'])
                updates['numero_processo'] = pii_manager.get_or_create_pii_mapping(original_np, "CASE_NUMBER", original_np)
            ...
            for key, pii_type, id_type in [
                ...
                ('advogados_polo_ativo', "LAWYER_FULL_STRING", "LAWYER_ID_NORMALIZED"), # We store full string UUIDs here
                ('advogados_polo_passivo', "LAWYER_FULL_STRING", "LAWYER_ID_NORMALIZED") # We store full string UUIDs here
            ]:
                ...
                if json_str:
                    try:
                        ...
                        if not isinstance(original_list, list):
                            logger.warning(f"Field {key} in decisoes.id {row['id']} is not a JSON list. Skipping PII replacement for it. Value: {json_str[:100]}")
                            continue

                        uuid_list = []
                        for item_str in original_list:
                            item_str_clean = str(item_str).strip()
                            if not item_str_clean: continue
                            ...
                            if id_type == "LAWYER_ID_NORMALIZED":
                                ...
                                if normalized_lawyer_id:
                                     pii_manager.get_or_create_pii_mapping(normalized_lawyer_id, id_type, normalized_lawyer_id)

                            # All items are mapped using their respective PII type
                            uuid_list.append(pii_manager.get_or_create_pii_mapping(item_str_clean, pii_type, item_str_clean))
                        updates[key] = json.dumps(uuid_list)
                    except json.JSONDecodeError:
                        logger.warning(f"Failed to parse JSON for field {key} in decisoes.id {row['id']}. Value: {json_str[:100]}")
                    except Exception as e_json:
                        logger.error(f"Error processing JSON field {key} for {row['id']}: {e_json}", exc_info=True)
            ...
            if raw_json_str:
                try:
                    ...
                    if raw_data.get('numero_processo'):
                        raw_data['numero_processo'] = pii_manager.get_or_create_pii_mapping(str(raw_data['numero_processo']), "CASE_NUMBER", str(raw_data['numero_processo']))

                    for polo_key in ['polo_ativo', 'polo_passivo']:
                        if raw_data.get(polo_key):
                            polo_list = raw_data[polo_key]
                            if isinstance(polo_list, str): polo_list = [polo_list] # handle single string
                            if isinstance(polo_list, list):
                                raw_data[polo_key] = [pii_manager.get_or_create_pii_mapping(str(name), "PARTY_NAME", str(name)) for name in polo_list if name and str(name).strip()]

                    for adv_key in ['advogados_polo_ativo', 'advogados_polo_passivo']:
                         if raw_data.get(adv_key):
                            adv_list = raw_data[adv_key]
                            if isinstance(adv_list, str): adv_list = [adv_list] # handle single string
                            if isinstance(adv_list, list):
                                temp_adv_uuids = []
                                for adv_name_str in adv_list:
                                    adv_name_str_clean = str(adv_name_str).strip()
                                    if not adv_name_str_clean: continue
                                    # Ensure normalized ID is mapped
                                    norm_id = normalize_lawyer_name(adv_name_str_clean)
                                    if norm_id:
                                        pii_manager.get_or_create_pii_mapping(norm_id, "LAWYER_ID_NORMALIZED", norm_id)
                                    # Store full string UUID in raw_json_data
                                    temp_adv_uuids.append(pii_manager.get_or_create_pii_mapping(adv_name_str_clean, "LAWYER_FULL_STRING", adv_name_str_clean))
                                raw_data[adv_key] = temp_adv_uuids
                    updates['raw_json_data'] = json.dumps(raw_data)
                except json.JSONDecodeError:
                    logger.warning(f"Failed to parse JSON for raw_json_data in decisoes.id {row['id']}. Value: {raw_json_str[:100]}")
                except Exception as e_rawjson:
                    logger.error(f"Error processing raw_json_data for {row['id']}: {e_rawjson}", exc_info=True)
            ...
            if updates and not dry_run:
                ...
                try:
                    ...
                    logger.error(f"Failed to update decisoes.id {row['id']}: {e_update}", exc_info=True)

            elif updates and dry_run:
                logger.info(f"DRY-RUN: Would update decisoes.id {row['id']} with {len(updates)} changes.")
...
def migrate_partidas_table(db: CausaGanhaDB, pii_manager: PiiManager, dry_run: bool = False):
    ...
    try:
        ...
        partida_ids = [row[0] for row in db.conn.execute("SELECT id FROM partidas ORDER BY id").fetchall()]
        ...
        for i, record_id in enumerate(partida_ids):
            row_data_tuple = db.conn.execute("SELECT * FROM partidas WHERE id = ?", (record_id,)).fetchone()
            if not row_data_tuple:
            ...
            # 1. numero_processo
            if row.get('numero_processo') and not 'uuid' in str(row.get('numero_processo')).lower():
                original_np = str(row['numero_processo'])
                updates['numero_processo'] = pii_manager.get_or_create_pii_mapping(original_np, "CASE_NUMBER", original_np)
            ...
            for key in ['equipe_a_ids', 'equipe_b_ids']:
                ...
                if json_str:
                    try:
                        original_list = json.loads(json_str)
                        if not isinstance(original_list, list):
                            logger.warning(f"Field {key} in partidas.id {row['id']} is not a JSON list. Skipping PII replacement. Value: {json_str[:100]}")
                            continue
                        # These are normalized lawyer names, map them to LAWYER_ID_NORMALIZED UUIDs
                        uuid_list = [
                            pii_manager.get_or_create_pii_mapping(str(norm_name), "LAWYER_ID_NORMALIZED", str(norm_name))
                            for norm_name in original_list if norm_name and str(norm_name).strip()
                        ]
                        updates[key] = json.dumps(uuid_list)
                    except json.JSONDecodeError:
                         logger.warning(f"Failed to parse JSON for field {key} in partidas.id {row['id']}. Value: {json_str[:100]}")
                    except Exception as e_json_eq:
                        logger.error(f"Error processing JSON field {key} for {row['id']}: {e_json_eq}", exc_info=True)


            # 3. Ratings dicts (keys are normalized lawyer names)
            for key in ['ratings_equipe_a_antes', 'ratings_equipe_b_antes', 'ratings_equipe_a_depois', 'ratings_equipe_b_depois']:
                json_str = row.get(key)
                if json_str:
                    try:
                        original_dict = json.loads(json_str)
                        if not isinstance(original_dict, dict):
                            logger.warning(f"Field {key} in partidas.id {row['id']} is not a JSON dict. Skipping PII replacement. Value: {json_str[:100]}")
                            continue

                        uuid_keyed_dict = {
                            pii_manager.get_or_create_pii_mapping(str(norm_name_key), "LAWYER_ID_NORMALIZED", str(norm_name_key)): value
                            for norm_name_key, value in original_dict.items() if norm_name_key and str(norm_name_key).strip()
                        }
                        updates[key] = json.dumps(uuid_keyed_dict)
                    except json.JSONDecodeError:
                        logger.warning(f"Failed to parse JSON for field {key} in partidas.id {row['id']}. Value: {json_str[:100]}")
                    except Exception as e_json_ratings:
                         logger.error(f"Error processing JSON field {key} for {row['id']}: {e_json_ratings}", exc_info=True)
            ...
            if updates and not dry_run:
                ...
                try:
                    ...
                    logger.error(f"Failed to update partidas.id {row['id']}: {e_update_partida}", exc_info=True)

            elif updates and dry_run:
                logger.info(f"DRY-RUN: Would update partidas.id {row['id']} with {len(updates)} changes.")
    ...
    try:
        ...
        if not args.dry_run:
            ...
            try:
                ...
                logger.warning(f"Could not explicitly begin transaction (may be normal for DuckDB): {e_tx_begin}")
