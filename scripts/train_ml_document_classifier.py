...
from causaganha.analysis.ml_document_classifier import MLDocumentEnsemble, DEFAULT_DOC_TYPE_PATH, DEFAULT_PROC_CLASS_PATH
...
def main() -> int:
    ...
    if not classif_docs_file.exists():
        logger.error("classificacoes_documentos_missing", hint="Run scripts/classify_documents.py first.")
        return 1
    ...
    doc_type_ensemble = MLDocumentEnsemble(target_col="document_type", ensemble_path=DEFAULT_DOC_TYPE_PATH)
    doc_type_cv = doc_type_ensemble.train(merged_df, embeddings)
    doc_type_ensemble.save()
    
    # Step 5: Train Procedural Class Ensemble
    logger.info("training_procedural_class_ensemble")
    proc_class_ensemble = MLDocumentEnsemble(target_col="procedural_class", ensemble_path=DEFAULT_PROC_CLASS_PATH)
    proc_class_cv = proc_class_ensemble.train(merged_df, embeddings)
