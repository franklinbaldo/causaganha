...
from causaganha.analysis.bayesian_fusion import OUTCOME_KEYS
...
def main() -> int:
    ...
    # Step 2: Merge data
    # We want: numero_processo (from comunicacoes), texto (from textos), outcome & confidence (from classificacoes)
    ...
    # Expected schema in ANCHOR_SCHEMA:
    # "numero_processo": str,
    ...
    # "embedding": object,     # bytes or list[float]
    ...
    for idx, (_, row) in enumerate(final_df.iterrows()):
        ...
        # Save embedding as float32 bytes for storage efficiency, matching AnchorClassifier.add_anchor
