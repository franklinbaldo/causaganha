...
# Kaggle CLI 2.x's authenticate() reads KAGGLE_API_TOKEN (or
# ~/.kaggle/access_token), not the older KAGGLE_KEY/KAGGLE_USERNAME pair —
# verified live: a KGAT_-prefixed token under KAGGLE_KEY alone fails with
# "Authentication required" until re-exported as KAGGLE_API_TOKEN. Kept as
# a fallback so existing KAGGLE_KEY-based .env files keep working without
# forcing a rename.
...
# visible to a bare system `python` — verified live (this failed once with
# ModuleNotFoundError before adding --with kaggle here and below).
KAGGLE_USERNAME="$(uv run --no-project --with kaggle python -c "
...
if uv run --no-project --with kaggle python -c "
from kaggle.api.kaggle_api_extended import KaggleApi
api = KaggleApi()
api.authenticate()
...
uv run --no-project --with kaggle python - "$KAGGLE_USERNAME/$DATASET_SLUG" <<'PY'
import sys, time
from kaggle.api.kaggle_api_extended import KaggleApi
...
echo "==> generating kernel script (opf_shared.py + config + kaggle body)"
{
  ...
  python - "$DATASET_SLUG" "$KAGGLE_USERNAME" "$EPOCHS" "$BATCH_SIZE" "$GPU" "$JOB_TYPE" "$OPF_PIN_COMMIT" "$REPO_COMMIT" "$WANDB_PIN" <<'PY'
import sys
...
uv run --no-project --with kaggle python - "$KAGGLE_USERNAME/$KERNEL_SLUG" <<'PY'
import sys, time
from kaggle.api.kaggle_api_extended import KaggleApi
