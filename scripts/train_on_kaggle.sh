...
echo "==> generating kernel script (opf_shared.py + config + kaggle body)"
{
  ...
  python3 - "$DATASET_SLUG" "$EPOCHS" "$BATCH_SIZE" "$GPU" "$JOB_TYPE" "$OPF_PIN_COMMIT" "$REPO_COMMIT" "$WANDB_PIN" <<'PY'
import sys
slug, epochs, batch, gpu, job_type, opf_commit, repo_commit, wandb_pin = sys.argv[1:]
