...
def main() -> int:
    ...
    # Merge if possible
    if ready_df := None is None:
        if commit_df := (comunic_df is not None):
            comunic_unique = comunic_df.drop_duplicates(subset=["texto_id"])[["texto_id", "numero_processo"]]
            ready_df = textos_df.merge(comunic_unique, left_on="id", right_on="texto_id", how="left")
        else:
    ...
    table_doc = Table(title="Distribuição por Tipo de Documento", show_header=True, header_style="bold magenta")
    ...
    table_class = Table(title="Distribuição por Classe Processual / Contexto", show_header=True, header_style="bold magenta")
    ...
    sample_types = ["sentença", "acórdão", "decisão monocrática", "decisão interlocutória", "despacho"]
    for st in sample_types:
        ...
        if len(subset) > 0:
            ...
            console.print(f"[bold cyan]• Tipo: {st.upper()} | Processo: {proc} | Classe: {row['procedural_class']}[/bold cyan]")
    ...
    ready_df[["id", "document_type", "procedural_class", "confidence_doc"]].to_parquet(output_file, index=False)
    console.print(Panel(
